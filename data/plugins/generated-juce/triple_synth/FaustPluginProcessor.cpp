...
typedef void (* staticInitFun) (dsp_imp* base_dsp, int sample_rate);
 ...
 *
 * Ownership:
 * - Owns the JSONUIDecoder created from a JSON string or from an existing base_dsp instance.
 ...
 * given duration (expressed in seconds) before applying them to the decorated base_dsp.
 *
 * Design notes
 * - Goal: wraps an existing base_dsp instance and transparently smooths its input controllers
 ...
 *   duration.
 *
 * - Reset behaviour: instanceResetUserInterface() resets both the wrapped base_dsp UI and the decoder
...
class smoothing_dsp : public decorator_dsp {
    ...
    protected:
        ...
        // Helper UI that only collects input control zones from the wrapped base_dsp.
        ...
        // Collect raw zones from the wrapped base_dsp UI for direct writes.
        void collectControlZones(::base_dsp* base_dsp)
        {
            ControlCollector collector;
            base_dsp->buildUserInterface(&collector);
        ...
        // Reinitialize smoothing buffers and push initial values to the wrapped base_dsp.
    ...
    public:
        ...
        // Store samplerate while forwarding constants to wrapped base_dsp.
        virtual void instanceConstants(int sample_rate)
        {
            decorator_dsp::instanceConstants(sample_rate);
            updateSmoothingSamples();
        }
        // Reset both decoder UI and wrapped base_dsp UI, then realign smoothing buffers.
        virtual void instanceResetUserInterface()
        {
        ...
        // Clear wrapped base_dsp state and stop any ongoing ramp.
        virtual void instanceClear()
        {
...
/************************** END poly-base_dsp.h **************************/
#ifndef PLUGIN_MAGIC
/* (removed unused code - see gen-juce.py) */
#endif
...
struct mydsp : public base_dsp {
	...
	void metadata(Meta* m) {
		...
		m->declare("compile_options", "-a /usr/share/faust/juce/juce-plugin.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -scn base_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp2 = 0.5f * fRec3[0] * (ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec1[0]), 65535))] + -1.0f) + 1.0f;
			...
			float fTemp12 = fRec20[0] * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec18[0]), 65535))];
			...
			float fTemp24 = mydsp_faustpower2_f(2.0f * fRec22[0] + -1.0f);
			fVec3[0] = fTemp24;
			...
			float fTemp42 = mydsp_faustpower2_f(2.0f * fRec26[0] + -1.0f);
			fVec5[0] = fTemp42;
			...
			float fTemp54 = fTemp53 * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec30[0]), 65535))] + fConst15 * fRec29[0] * fRec28[0] * fTemp13;
			...
			fRec9[0] = fSlow18 * (fTemp56 * (fTemp53 * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec32[0]), 65535))] + fConst15 * fRec29[0] * fRec31[0] * fTemp33 + fTemp54) + fRec27[0] * (fConst14 * fTemp31 * fTemp47 + fRec21[0] * (2.0f * fRec24 + -1.0f) + fTemp32)) * fTemp10 - (fRec9[2] * fTemp8 + 2.0f * fRec9[1] * fTemp7) / fTemp6;
			...
			float fTemp67 = mydsp_faustpower2_f(2.0f * fRec39[0] + -1.0f);
			fVec8[0] = fTemp67;
			...
			fRec36[0] = fSlow18 * fTemp10 * (fTemp56 * (fTemp54 + fTemp53 * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec41[0]), 65535))] + fConst15 * fRec29[0] * fRec40[0] * fTemp58) + fRec27[0] * (fTemp32 + fConst14 * fTemp31 * fTemp72 + fRec21[0] * (2.0f * fRec38 + -1.0f))) - (fTemp8 * fRec36[2] + 2.0f * fTemp7 * fRec36[1]) / fTemp6;
	...
	#define FAUST_FILE_NAME "triple_synth.dsp"
	#define FAUST_CLASS_NAME "mydsp"
	#define FAUST_COMPILATION_OPIONS "-a /usr/share/faust/juce/juce-plugin.cpp -lang cpp -i -fpga-mem-th 4 -ct 1 -scn base_dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
	#define FAUST_INPUTS 0
	#define FAUST_OUTPUTS 2
	#define FAUST_ACTIVES 16
	#define FAUST_PASSIVES 0

	FAUST_ADDNUMENTRY("Synth/Pitchwheel", fEntry1, 0.0f, -2.0f, 2.0f, 0.001f);
	FAUST_ADDNUMENTRY("Synth/Waveform", fEntry2, 3.0f, 0.0f, 3.0f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("Synth/[0] Envelope/Attack", fHslider5, 0.03f, 0.001f, 2.0f, 0.001f);
	FAUST_ADDHORIZONTALSLIDER("Synth/[0] Envelope/Decay", fHslider6, 0.3f, 0.001f, 4.0f, 0.001f);
	FAUST_ADDHORIZONTALSLIDER("Synth/[0] Envelope/Sustain", fHslider7, 1.0f, 0.0f, 1.0f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("Synth/[0] Envelope/Release", fHslider4, 0.4f, 0.001f, 4.0f, 0.001f);
	FAUST_ADDNUMENTRY("Synth/[0] Envelope/gain", fEntry3, 1.0f, 0.0f, 1.0f, 0.01f);
	FAUST_ADDBUTTON("Synth/[0] Envelope/gate", fButton0);
	FAUST_ADDHORIZONTALSLIDER("Synth/Tremolo Freq", fHslider0, 0.0f, 0.0f, 2e+01f, 0.02f);
	FAUST_ADDHORIZONTALSLIDER("Synth/Tremolo Depth", fHslider1, 0.0f, 0.0f, 1.0f, 0.001f);
	FAUST_ADDHORIZONTALSLIDER("Synth/[1] Filter/Cutoff", fHslider2, 1e+04f, 1.1e+02f, 2e+04f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("Synth/[1] Filter/Cutoff Q", fHslider3, 1.0f, 1.0f, 2e+01f, 0.1f);
	FAUST_ADDHORIZONTALSLIDER("Synth/Detune", fHslider10, 0.1f, 0.0f, 1.0f, 0.001f);
	FAUST_ADDHORIZONTALSLIDER("Synth/Vibrato Freq", fHslider8, 0.0f, 0.0f, 2e+01f, 0.02f);
	FAUST_ADDHORIZONTALSLIDER("Synth/Vibrato Depth", fHslider9, 0.0f, 0.0f, 1.0f, 0.001f);
	FAUST_ADDNUMENTRY("Synth/freq", fEntry0, 2e+02f, 4e+01f, 2e+03f, 0.01f);

	#define FAUST_LIST_ACTIVES(p) \
		p(NUMENTRY, Pitchwheel, "Synth/Pitchwheel", fEntry1, 0.0f, -2.0f, 2.0f, 0.001f) \
		p(NUMENTRY, Waveform, "Synth/Waveform", fEntry2, 3.0f, 0.0f, 3.0f, 1.0f) \
		p(HORIZONTALSLIDER, Attack, "Synth/[0] Envelope/Attack", fHslider5, 0.03f, 0.001f, 2.0f, 0.001f) \
		p(HORIZONTALSLIDER, Decay, "Synth/[0] Envelope/Decay", fHslider6, 0.3f, 0.001f, 4.0f, 0.001f) \
		p(HORIZONTALSLIDER, Sustain, "Synth/[0] Envelope/Sustain", fHslider7, 1.0f, 0.0f, 1.0f, 0.01f) \
		p(HORIZONTALSLIDER, Release, "Synth/[0] Envelope/Release", fHslider4, 0.4f, 0.001f, 4.0f, 0.001f) \
		p(NUMENTRY, gain, "Synth/[0] Envelope/gain", fEntry3, 1.0f, 0.0f, 1.0f, 0.01f) \
		p(BUTTON, gate, "Synth/[0] Envelope/gate", fButton0, 0.0f, 0.0f, 1.0f, 1.0f) \
		p(HORIZONTALSLIDER, Tremolo_Freq, "Synth/Tremolo Freq", fHslider0, 0.0f, 0.0f, 2e+01f, 0.02f) \
		p(HORIZONTALSLIDER, Tremolo_Depth, "Synth/Tremolo Depth", fHslider1, 0.0f, 0.0f, 1.0f, 0.001f) \
		p(HORIZONTALSLIDER, Cutoff, "Synth/[1] Filter/Cutoff", fHslider2, 1e+04f, 1.1e+02f, 2e+04f, 0.01f) \
		p(HORIZONTALSLIDER, Cutoff_Q, "Synth/[1] Filter/Cutoff Q", fHslider3, 1.0f, 1.0f, 2e+01f, 0.1f) \
		p(HORIZONTALSLIDER, Detune, "Synth/Detune", fHslider10, 0.1f, 0.0f, 1.0f, 0.001f) \
		p(HORIZONTALSLIDER, Vibrato_Freq, "Synth/Vibrato Freq", fHslider8, 0.0f, 0.0f, 2e+01f, 0.02f) \
		p(HORIZONTALSLIDER, Vibrato_Depth, "Synth/Vibrato Depth", fHslider9, 0.0f, 0.0f, 1.0f, 0.001f) \
		p(NUMENTRY, freq, "Synth/freq", fEntry0, 2e+02f, 4e+01f, 2e+03f, 0.01f) \
