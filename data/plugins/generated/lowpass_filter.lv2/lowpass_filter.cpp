...
Compilation options: -a /usr/share/faust/lv2.cpp -lang cpp -i -ct 1 -cn lowpass_filter -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 32
------------------------------------------------------------ */
...
class lowpass_filter : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /usr/share/faust/lv2.cpp -lang cpp -i -ct 1 -cn lowpass_filter -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 32");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (count - 32); vindex = vindex + 32) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fZec4[i] = (fZec1[i] + -1.4142135f) / fZec0[i] + 1.0f;
			}
			/* Recursive loop 6 */
			/* Pre code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fRec0[i] = float(input0[i]) - (fRec0[i - 2] * fZec4[i] + 2.0f * fRec0[i - 1] * fZec3[i]) / fZec2[i];
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fRec2[i] = float(input1[i]) - (fZec4[i] * fRec2[i - 2] + 2.0f * fZec3[i] * fRec2[i - 1]) / fZec2[i];
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				output1[i] = FAUSTFLOAT((fRec2[i - 2] + fRec2[i] + 2.0f * fRec2[i - 1]) / fZec2[i]);
			}
		}
		/* Remaining frames */
		if (vindex < count) {
