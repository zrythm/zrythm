...
Compilation options: -a /usr/share/faust/lv2.cpp -lang cpp -i -ct 1 -cn cubic_distortion -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 32
------------------------------------------------------------ */
...
class cubic_distortion : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /usr/share/faust/lv2.cpp -lang cpp -i -ct 1 -cn cubic_distortion -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 32");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->declare(&fHslider1, "tooltip", "Amount of distortion");
		...
		ui_interface->addHorizontalSlider("Offset", &fHslider0, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.01f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (count - 32); vindex = vindex + 32) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fZec0[i] = std::max<float>(-1.0f, std::min<float>(1.0f, fRec1[i] + float(input0[i]) * std::pow(1e+01f, 2.0f * fRec2[i])));
			}
			/* Vectorizable loop 3 */
			/* Pre code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fYec0[i] = fZec0[i] * (1.0f - 0.33333334f * cubic_distortion_faustpower2_f(fZec0[i]));
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fRec0[i] = 0.995f * fRec0[i - 1] + fYec0[i] - fYec0[i - 1];
			}
			/* Post code */
...
#ifndef PLUGIN_URI
#define PLUGIN_URI URI_PREFIX "/cubic_distortion"
#endif
...
struct LV2Plugin {
  ...
  static void init_meta()
  {
    if (!meta && (meta = new Meta)) {
      ...
      cubic_distortion* tmp_dsp = new cubic_distortion();
      if (tmp_dsp) {
  ...
  {
    return meta_get("name", "cubic_distortion");
  }
      ...
      vd(num_voices>0?new VoiceData(num_voices):0)
  {
    ...
    dsp = (cubic_distortion**)calloc(ndsps, sizeof(cubic_distortion*));
...
#endif
  if (!plugin_name || !*plugin_name) plugin_name = "cubic_distortion";
