...
Compilation options: -a /usr/share/faust/lv2.cpp -lang cpp -i -cn white_noise -es 1 -mcd 16 -single -ftz 0 -vec -lv 0 -vs 32
------------------------------------------------------------ */
...
class white_noise : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /usr/share/faust/lv2.cpp -lang cpp -i -cn white_noise -es 1 -mcd 16 -single -ftz 0 -vec -lv 0 -vs 32");
	...
	virtual white_noise* clone() {
		return new white_noise();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("Amp", &fHslider0, FAUSTFLOAT(-10.0f), FAUSTFLOAT(-70.0f), FAUSTFLOAT(10.0f), FAUSTFLOAT(0.100000001f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = fConst0 * std::pow(10.0f, 0.0500000007f * float(fHslider0));
		float fRec0_tmp[36];
		...
		for (vindex = 0; vindex <= count - 32; vindex = vindex + 32) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fZec0[i] = 4.65661287e-10f * fRec0[i] * float(iRec1[i]);
			}
			/* Vectorizable loop 3 */
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
...
#ifndef PLUGIN_URI
#define PLUGIN_URI URI_PREFIX "/white_noise"
#endif
...
struct LV2Plugin {
  ...
  static void init_meta()
  {
    if (!meta && (meta = new Meta)) {
      ...
      white_noise* tmp_dsp = new white_noise();
      if (tmp_dsp) {
  ...
  {
    return meta_get("name", "white_noise");
  }
      ...
      vd(num_voices>0?new VoiceData(num_voices):0)
  {
    ...
    dsp = (white_noise**)calloc(ndsps, sizeof(white_noise*));
...
#endif
  if (!plugin_name || !*plugin_name) plugin_name = "white_noise";
