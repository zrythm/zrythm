...
#ifndef FAUSTCLASS 
#define FAUSTCLASS highpass_filter
#endif
...
static float highpass_filter_faustpower2_f(float value) {
	return value * value;
}

class highpass_filter : public dsp {
 ...
 public:
	...
	virtual highpass_filter* clone() {
		return new highpass_filter();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= count - 32; vindex = vindex + 32) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fZec3[i] = highpass_filter_faustpower2_f(fZec0[i]);
			}
			/* Vectorizable loop 4 */
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
...
#ifndef PLUGIN_URI
#define PLUGIN_URI URI_PREFIX "/highpass_filter"
#endif
...
struct LV2Plugin {
  ...
  static void init_meta()
  {
    if (!meta && (meta = new Meta)) {
      ...
      highpass_filter* tmp_dsp = new highpass_filter();
      if (tmp_dsp) {
  ...
  {
    return meta_get("name", "highpass_filter");
  }
      ...
      vd(num_voices>0?new VoiceData(num_voices):0)
  {
    ...
    dsp = (highpass_filter**)calloc(ndsps, sizeof(highpass_filter*));
...
#endif
  if (!plugin_name || !*plugin_name) plugin_name = "highpass_filter";
