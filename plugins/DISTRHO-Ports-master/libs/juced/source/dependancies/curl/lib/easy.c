...
CURLcode curl_easy_perform(CURL *curl)
{
  ...
  if( ! (data->share && data->share->hostcache) ) {
    ...
    if(!data->dns.hostcache) {
      data->dns.hostcachetype = Names::HCACHE_PRIVATE;
  ...
  if(!data->state.connc) {
    ...
    data->state.connc = Curl_mk_connc(conncache::CONNCACHE_PRIVATE, -1);
    if(!data->state.connc)
      return CURLE_OUT_OF_MEMORY;
  }
                        ...
                        void *multi)
{
  ...
  if(multi == NULL)
    /* the association is cleared, mark the easy handle as not used by an
       interface */
    data->state.used_interface = UrlState::Curl_if_none;
}
  ...
  if(fail) {
    if(outcurl) {
      if(outcurl->state.connc &&
         (outcurl->state.connc->type == conncache::CONNCACHE_PRIVATE))
...
CURLcode curl_easy_recv(CURL *curl, void *buffer, size_t buflen, size_t *n)
{
  ...
  *n = 0;
  ret1 = Curl_read(c, sfd, (char*) buffer, buflen, &n1);
