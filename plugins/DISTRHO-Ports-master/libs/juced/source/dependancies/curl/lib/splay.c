 ...
 */
#define curl_splay_compare_keys(i,j) Curl_splaycomparekeys((i),(j))
                             ...
                             struct Curl_tree *t)
{
  ...
  for (;;) {
    comp = curl_splay_compare_keys(i, t->key);
    if(comp < 0) {
      if(t->smaller == NULL)
        break;
      if(curl_splay_compare_keys(i, t->smaller->key) < 0) {
      ...
      if(t->larger == NULL)
        break;
      if(curl_splay_compare_keys(i, t->larger->key) > 0) {
                                   ...
                                   struct Curl_tree *node)
{
  ...
  if(t != NULL) {
    t = Curl_splay(i,t);
    if(curl_splay_compare_keys(i, t->key)==0) {
  ...
  else if(curl_splay_compare_keys(i, t->key) < 0) {
    node->smaller = t->smaller;
    node->larger = t;
    t->smaller = NULL;
                                   ...
                                   struct Curl_tree **removed)
{
  ...
  *removed = NULL; /* default to no removed */
  ...
  t = Curl_splay(i,t);
  if(curl_splay_compare_keys(i, t->key) == 0) {               /* found it */
                           ...
                           struct Curl_tree **newroot)
{
  ...
  if(curl_splay_compare_keys(KEY_NOTUSED, removenode->key) == 0) {
  ...
  /* First make sure that we got the same root node as the one we want
     ...
     We cannot just curl_splay_compare_keys the keys here as a double remove in quick
