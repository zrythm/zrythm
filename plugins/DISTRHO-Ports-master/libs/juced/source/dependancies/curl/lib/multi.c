...
static struct Curl_sh_entry *sh_addentry(struct curl_hash *sh,
                                         ...
                                         struct SessionHandle *data)
{
  struct Curl_sh_entry *there = (struct Curl_sh_entry *) 
  ...
  /* not present, add it */
  check = (struct Curl_sh_entry *) calloc(sizeof(struct Curl_sh_entry), 1);
  if(!check)
...
{
  struct Curl_multi *multi = (struct Curl_multi *) calloc(sizeof(struct Curl_multi), 1);
  ...
  multi->connc = Curl_mk_connc(conncache::CONNCACHE_MULTI, -1);
  if(!multi->connc) {
                                ...
                                CURL *easy_handle)
{
  ...
  if(easy->easy_handle->dns.hostcache &&
      (easy->easy_handle->dns.hostcachetype == Names::HCACHE_PRIVATE)) {
    Curl_hash_destroy(easy->easy_handle->dns.hostcache);
    easy->easy_handle->dns.hostcache = NULL;
    easy->easy_handle->dns.hostcachetype = Names::HCACHE_NONE;
  }

  if(!easy->easy_handle->dns.hostcache ||
      (easy->easy_handle->dns.hostcachetype == Names::HCACHE_NONE)) {
    easy->easy_handle->dns.hostcache = multi->hostcache;
    easy->easy_handle->dns.hostcachetype = Names::HCACHE_MULTI;
  }

  if(easy->easy_handle->state.connc) {
    if(easy->easy_handle->state.connc->type == conncache::CONNCACHE_PRIVATE) {
  ...
  easy->easy_handle->state.connc->type = conncache::CONNCACHE_MULTI;
                                   ...
                                   CURL *curl_handle)
{
  ...
  if(easy) {
    ...
    if(easy->easy_handle->dns.hostcachetype == Names::HCACHE_MULTI) {
    ...
    if(easy->easy_handle->state.connc->type == conncache::CONNCACHE_MULTI) {
  ...
  do {
    ...
    case CURLM_STATE_INIT:
      ...
      if(CURLE_OK == easy->result) {
        ...
        easy->easy_handle->state.used_interface = UrlState::Curl_if_multi;
      }
      break;
                              ...
                              int *running_handles)
{
  ...
  if(checkall) {
    ...
    struct Curl_sh_entry *entry = (struct Curl_sh_entry*) 
