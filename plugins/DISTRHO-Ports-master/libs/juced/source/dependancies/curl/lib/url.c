...
CURLcode Curl_close(struct SessionHandle *data)
{
  ...
  if(data->state.connc) {
    ...
    if(data->state.connc->type == conncache::CONNCACHE_PRIVATE) {
  ...
  if(data->dns.hostcachetype == Names::HCACHE_PRIVATE) {
                                ...
                                long amount) /* set -1 to use default */
{
  ...
  if(type == conncache::CONNCACHE_PRIVATE) {
  ...
  c = (struct conncache*) calloc(sizeof(struct conncache), 1);
  if(!c)
    return NULL;
  ...
  c->connects = (struct connectdata **) calloc(sizeof(struct connectdata *), (size_t)default_amount);
  if(!c->connects) {
    free(c);
    return NULL;
  }
                       ...
                       long newamount)
{
  ...
  if(!c) {
    ...
    */
    data->state.connc = Curl_mk_connc(conncache::CONNCACHE_PRIVATE, newamount);
    if(!data->state.connc)
      return CURLE_OUT_OF_MEMORY;
    return CURLE_OK;
  }
