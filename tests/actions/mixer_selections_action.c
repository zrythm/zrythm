  ...
  bool         with_carla)
{
  ...
  num_master_children++;
  g_assert_cmpint (
    P_MASTER_TRACK->num_children, ==, num_master_children);
...
#  ifdef HAVE_CARLA
  _test_copy_plugins (
    NO_DELAY_LINE_BUNDLE, NO_DELAY_LINE_URI, false, true);
#  endif /* HAVE_CARLA */
  ...
  bool         with_carla)
{
  ...
  mixer_selections_clear (
    MIXER_SELECTIONS, F_NO_PUBLISH_EVENTS);
  mixer_selections_add_slot (
    MIXER_SELECTIONS, src_track, PLUGIN_SLOT_INSERT, 0,
  ...
  mixer_selections_add_slot (
    MIXER_SELECTIONS, src_track, PLUGIN_SLOT_INSERT, 1,
    F_NO_CLONE, F_NO_PUBLISH_EVENTS);
...
test_move_two_plugins_one_slot_up (void)
{
  ...
  ret = mixer_selections_action_perform_copy (
    MIXER_SELECTIONS, PORT_CONNECTIONS_MGR,
    PLUGIN_SLOT_INSERT, track_get_name_hash (track), 1, NULL);
  g_assert_true (ret);
  undo_manager_undo (UNDO_MANAGER, NULL);
  undo_manager_redo (UNDO_MANAGER, NULL);
  ...
  ret = mixer_selections_action_perform_move (
    MIXER_SELECTIONS, PORT_CONNECTIONS_MGR,
    PLUGIN_SLOT_INSERT, track_get_name_hash (track), 2, NULL);
  g_assert_true (ret);
  g_assert_true (track_validate (track));
  undo_manager_undo (UNDO_MANAGER, NULL);
  g_assert_true (track_validate (track));
  ...
  ret = mixer_selections_action_perform_move (
    MIXER_SELECTIONS, PORT_CONNECTIONS_MGR,
    PLUGIN_SLOT_INSERT, track_get_name_hash (track), 0, NULL);
  g_assert_true (ret);
  undo_manager_undo (UNDO_MANAGER, NULL);
  undo_manager_redo (UNDO_MANAGER, NULL);
   ...
   * plugin */
  g_assert_true (
    IS_PLUGIN_AND_NONNULL (track->channel->inserts[0]));
...
#  ifdef HAVE_MIDI_CC_MAP
  ...
  g_assert_cmpstr (
    pl->setting->descr->uri, ==, setting->descr->uri);
  port = plugin_get_port_by_symbol (pl, "ccin");
  g_assert_cmpfloat_with_epsilon (
    port->control, 120.f, 0.0001f);
...
#  ifdef HAVE_CARLA
  ...
  Plugin * p1 =
    P_MODULATOR_TRACK
      ->modulators[P_MODULATOR_TRACK->num_modulators - 2];
  Plugin * p2 =
    P_MODULATOR_TRACK
      ->modulators[P_MODULATOR_TRACK->num_modulators - 1];
  Port * cv_out = NULL;
  ...
  mixer_selections_add_slot (
    ...
    P_MODULATOR_TRACK->num_modulators - 2, F_NO_CLONE,
    F_NO_PUBLISH_EVENTS);
  ret = mixer_selections_action_perform_delete (
...
test_move_pl_after_duplicating_track (void)
{
  ...
  mixer_selections_clear (
    MIXER_SELECTIONS, F_NO_PUBLISH_EVENTS);
  mixer_selections_add_slot (
    MIXER_SELECTIONS, lsp_track, PLUGIN_SLOT_INSERT, 0,
    F_NO_CLONE, F_NO_PUBLISH_EVENTS);
  ret = mixer_selections_action_perform_move (
    MIXER_SELECTIONS, PORT_CONNECTIONS_MGR, PLUGIN_SLOT_INSERT,
    track_get_name_hash (dest_track), 1, NULL);
...
test_move_plugin_from_inserts_to_midi_fx (void)
{
  ...
  ret = mixer_selections_action_perform_move (
    MIXER_SELECTIONS, PORT_CONNECTIONS_MGR,
    PLUGIN_SLOT_MIDI_FX, track_get_name_hash (track), 0, NULL);
  g_assert_true (ret);
...
#  endif
  ...
  src_track = TRACKLIST->tracks[src_track_pos];
  g_assert_true (
    IS_PLUGIN_AND_NONNULL (src_track->channel->instrument));
  ...
  mixer_selections_action_perform_create (
    PLUGIN_SLOT_INSTRUMENT, track_get_name_hash (src_track),
    -1, setting, 1, NULL);
  g_assert_true (track_validate (src_track));
