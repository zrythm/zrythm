...
#define RANGE_SIZE_IN_BARS (RANGE_END_BAR - RANGE_START_BAR) /* 6 */
...
#define ADD_MREGION(start_bar, end_bar) \
  ...
  success = arranger_object_resize (
    ...
    ArrangerObjectResizeType::ARRANGER_OBJECT_RESIZE_LOOP, missing_ticks, false,
    NULL);
  g_assert_true (success);
  arranger_selections_action_perform_resize (
    sel_before, (ArrangerSelections *) TL_SELECTIONS,
    ArrangerSelectionsActionResizeType::ARRANGER_SELECTIONS_ACTION_RESIZE_R_LOOP,
    missing_ticks, NULL);
...
check_start_end_markers (void)
{
  ...
  Marker * start_m = marker_track_get_start_marker (P_MARKER_TRACK);
  g_assert_true (start_m->type == MarkerType::MARKER_TYPE_START);
  ArrangerObject * start_obj = (ArrangerObject *) start_m;
  Marker *         end_m = marker_track_get_end_marker (P_MARKER_TRACK);
  g_assert_true (end_m->type == MarkerType::MARKER_TYPE_END);
  ...
  g_assert_true (position_is_after_or_equal (&start_obj->pos, &init_pos));
  g_assert_true (position_is_after_or_equal (&end_obj->pos, &init_pos));
}
...
#define CHECK_POS(obj, start_bar, end_bar) \
  ...
  Position start, end;
  CHECK_POS (midi_region_obj, MIDI_REGION_START_BAR, MIDI_REGION_END_BAR);
  CHECK_POS (midi_region2_obj, MIDI_REGION2_START_BAR, MIDI_REGION2_END_BAR);
  CHECK_POS (midi_region3_obj, MIDI_REGION3_START_BAR, MIDI_REGION3_END_BAR);
  CHECK_POS (midi_region4_obj, MIDI_REGION4_START_BAR, MIDI_REGION4_END_BAR);
  CHECK_POS (midi_region5_obj, MIDI_REGION5_START_BAR, MIDI_REGION5_END_BAR);
  CHECK_POS (midi_region6_obj, MIDI_REGION6_START_BAR, MIDI_REGION6_END_BAR);
  CHECK_POS (midi_region7_obj, MIDI_REGION7_START_BAR, MIDI_REGION7_END_BAR);
  CHECK_POS (audio_region_obj, AUDIO_REGION_START_BAR, AUDIO_REGION_END_BAR);
...
#define CHECK_POS(obj, start_bar, end_bar) \
  ...
  CHECK_POS (
    midi_region1_obj, MIDI_REGION_START_BAR + RANGE_SIZE_IN_BARS,
    MIDI_REGION_END_BAR + RANGE_SIZE_IN_BARS);
  CHECK_POS (
    midi_region2_obj, MIDI_REGION2_START_BAR + RANGE_SIZE_IN_BARS,
    MIDI_REGION2_END_BAR + RANGE_SIZE_IN_BARS);

  /* orig 3 */
  CHECK_POS (midi_region9_obj, MIDI_REGION3_START_BAR, RANGE_START_BAR);
  CHECK_POS (
    midi_region10_obj, RANGE_END_BAR, MIDI_REGION3_END_BAR + RANGE_SIZE_IN_BARS);

  /* orig 4 */
  CHECK_POS (midi_region7_obj, MIDI_REGION4_START_BAR, RANGE_START_BAR);
  CHECK_POS (
    midi_region8_obj, RANGE_END_BAR, MIDI_REGION4_END_BAR + RANGE_SIZE_IN_BARS);

  /* orig 5 */
  CHECK_POS (midi_region5_obj, MIDI_REGION5_START_BAR, RANGE_START_BAR);
  CHECK_POS (
    midi_region6_obj, RANGE_END_BAR, MIDI_REGION5_END_BAR + RANGE_SIZE_IN_BARS);

  /* orig 6 */
  CHECK_POS (
    midi_region3_obj, MIDI_REGION6_START_BAR + RANGE_SIZE_IN_BARS,
    MIDI_REGION6_END_BAR + RANGE_SIZE_IN_BARS);

  /* orig 7 */
  CHECK_POS (midi_region4_obj, MIDI_REGION7_START_BAR, MIDI_REGION7_END_BAR);
...
#undef CHECK_POS
  ...
  position_set_to_bar (&audio_region1_end_after_expected, RANGE_START_BAR);
  position_set_to_bar (&audio_region2_start_after_expected, RANGE_END_BAR);
  position_set_to_bar (
    &audio_region2_end_after_expected,
    AUDIO_REGION_END_BAR + RANGE_SIZE_IN_BARS);
  g_assert_cmppos (
    &audio_region_obj1->end_pos, &audio_region1_end_after_expected);
  g_assert_cmppos (&audio_region_obj2->pos, &audio_region2_start_after_expected);
  g_assert_cmppos (
    &audio_region_obj2->end_pos, &audio_region2_end_after_expected);
...
#define CHECK_TPOS(name, expected_bar) \
  ...
  CHECK_TPOS (playhead_pos, PLAYHEAD_BEFORE + RANGE_SIZE_IN_BARS);
  CHECK_TPOS (loop_end_pos, LOOP_END_BEFORE + RANGE_SIZE_IN_BARS);
...
test_insert_silence (void)
{
  ...
  UndoableAction * ua = range_action_new_insert_silence (&start, &end, NULL);

  /* verify that number of objects is as expected */
  RangeAction * ra = (RangeAction *) ua;
  g_assert_cmpint (
    arranger_selections_get_num_objects ((ArrangerSelections *) ra->sel_before),
    ==, 7);
...
#define CHECK_POS(obj, start_bar, end_bar) \
  ...
  /* orig 3 */
  CHECK_POS (midi_region8_obj, MIDI_REGION3_START_BAR, RANGE_START_BAR);
  ...
  /* orig 7 */
  CHECK_POS (midi_region3_obj, MIDI_REGION7_START_BAR, MIDI_REGION7_END_BAR);
...
#undef CHECK_POS
  ...
  position_set_to_bar (
    &audio_region_start_after_expected, AUDIO_REGION_START_BAR);
  position_set_to_bar (&audio_region_end_after_expected, RANGE_START_BAR);
  g_assert_cmppos (&audio_region_obj->pos, &audio_region_start_after_expected);
  g_assert_cmppos (&audio_region_obj->end_pos, &audio_region_end_after_expected);
...
test_remove_range_w_objects_inside (void)
{
  ...
  /* create scale object */
  MusicalScale * ms = musical_scale_new ((MusicalScaleType) 0, (MusicalNote) 0);
  ScaleObject *  so = scale_object_new (ms);
  chord_track_add_scale (P_CHORD_TRACK, so);
  arranger_object_select (
    (ArrangerObject *) so, F_SELECT, F_NO_APPEND, F_NO_PUBLISH_EVENTS);
...
#define TEST_PREFIX "/actions/range/"
  ...
  g_test_add_func (
    TEST_PREFIX "test insert silence", (GTestFunc) test_insert_silence);
  g_test_add_func (
    TEST_PREFIX "test remove range", (GTestFunc) test_remove_range);
