...
test_bounce_instrument_track (void)
{
  ...
  _test_bounce_instrument_track (BOUNCE_STEP_BEFORE_INSERTS, false);
  _test_bounce_instrument_track (BOUNCE_STEP_PRE_FADER, false);
  _test_bounce_instrument_track (BOUNCE_STEP_POST_FADER, false);
}
...
test_chord_routed_to_instrument (void)
{
  ...
  arranger_object_set_position (
    chord_obj, &start_pos, ARRANGER_OBJECT_POSITION_TYPE_START, F_NO_VALIDATE);
  ...
  for (int i = 0; i < 2; i++)
    {
      /* bounce */
      ExportSettings * settings = export_settings_new ();
      settings->mode = i == 0 ? EXPORT_MODE_FULL : EXPORT_MODE_TRACKS;
      export_settings_set_bounce_defaults (
        settings, EXPORT_FORMAT_WAV, NULL, __func__);
      settings->time_range = TIME_RANGE_LOOP;
