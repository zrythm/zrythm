...
public:
  ...
  {
    // Sizes are physical pixels (CLAP spec on X11/Win32), derived from the
    // logical base size and the scale last fed by the host
    *width = static_cast<uint32_t> (std::lround (kWidth * scale_));
    *height = static_cast<uint32_t> (std::lround (kHeight * scale_));
    return true;
  }
  ...
  // Hosts must pair shows and hides
