 ...
 *
 * Sizes are physical pixels derived from the logical base size and the
 * scale last fed via IPlugViewContentScaleSupport.
 */
class TestGuiView
    : public U::Implements<U::Directly<IPlugView, IPlugViewContentScaleSupport>>
{
public:
  ...
  tresult PLUGIN_API attached (void * parent, FIDString type) SMTG_OVERRIDE
  {
    if (frame_ != nullptr)
      {
        ViewRect new_size (
          0, 0, static_cast<int32> (std::lround (kResizedWidth * scale_)),
          static_cast<int32> (std::lround (kResizedHeight * scale_)));
  ...
  {
    // Rescale the current view size to the new scale
    const auto ratio = factor / scale_;
    scale_ = factor;
    ViewRect new_size (
      0, 0, static_cast<int32> (std::lround (size_.getWidth () * ratio)),
      static_cast<int32> (std::lround (size_.getHeight () * ratio)));
    if (frame_ != nullptr)
      frame_->resizeView (this, &new_size);
    else
      size_ = new_size;
