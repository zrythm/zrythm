...
TEST_F (PluginRunLoopTest, TimerStopsAfterUnregister)
{
  ...
  // Negative assertion: a timer that keeps firing is not pollable as a
  // positive condition, so this waits out a fixed window (at the 5 ms
  // period it would fire ~30 times if still active)
  EXPECT_FALSE (
...
TEST_F (PluginRunLoopTest, FdUpdateDisablesAndReenablesWatch)
{
  ...
  ASSERT_EQ (write (fds[1], "x", 1), 1);
  // Negative assertion (fixed wait is inherent): the fd stays readable, so
  // an active watch would fire within this window
