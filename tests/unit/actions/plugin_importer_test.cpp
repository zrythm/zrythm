...
TEST_F (PluginImporterTest, ImportNullDescriptor)
{
  ...
  plugin_importer_->importPlugin (nullptr);
...
TEST_F (PluginImporterTest, ImportInstrumentPlugin)
{
  ...
  plugin_importer_->importPlugin (descriptor.get ());
...
TEST_F (PluginImporterTest, ImportMultiplePluginTypes)
{
  ...
  plugin_importer_->importPlugin (instrument_desc.get ());
  QCoreApplication::processEvents ();

  // Import MIDI modifier plugin
  plugin_importer_->importPlugin (midi_modifier_desc.get ());
  QCoreApplication::processEvents ();

  // Import effect plugin
  plugin_importer_->importPlugin (effect_desc.get ());
  QCoreApplication::processEvents ();
...
TEST_F (PluginImporterTest, ImportDifferentProtocols)
{
  ...
  plugin_importer_->importPlugin (clap_descriptor.get ());
  QCoreApplication::processEvents ();
  EXPECT_TRUE (instantiation_finished_called_);
  ...
  plugin_importer_->importPlugin (vst3_descriptor.get ());
  QCoreApplication::processEvents ();
  EXPECT_TRUE (instantiation_finished_called_);
}
...
TEST_F (PluginImporterTest, MultipleImportsSameType)
{
  ...
  // Import first plugin
  plugin_importer_->importPlugin (descriptor1.get ());
  QCoreApplication::processEvents ();

  // Import second plugin
  plugin_importer_->importPlugin (descriptor2.get ());
  QCoreApplication::processEvents ();
...
TEST_F (PluginImporterTest, PluginFactoryFailure)
{
  ...
  EXPECT_NO_THROW (plugin_importer_->importPlugin (descriptor.get ()));
}
