...
/// Verifies that all registry sizes and IDs are equal between two serialized
/// projects
inline void
expect_registries_match (const nlohmann::json &j1, const nlohmann::json &j2)
{
  ...
  const std::array<std::string_view, 5> registry_names = {
    // "portRegistry",
    "paramRegistry",          "pluginRegistry",          "trackRegistry",
    "arrangerObjectRegistry", "fileAudioSourceRegistry",
  };
