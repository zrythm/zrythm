...
TEST_F (MetronomeTest, PrepareForProcessing)
{
  ...
  // Should not throw
  EXPECT_NO_THROW (metronome->prepare_for_processing (44100, 256));
}
...
TEST_F (MetronomeTest, BarAndBeatTicks)
{
  ...
  tempo_map_->add_time_signature_event (0, 4, 4);
  tempo_map_->add_tempo_event (0, 120.0, TempoMap::CurveType::Constant);
...
TEST_F (MetronomeTest, DifferentTimeSignatures)
{
  ...
  tempo_map_->add_time_signature_event (0, 3, 4);
  tempo_map_->add_tempo_event (0, 120.0, TempoMap::CurveType::Constant);
...
TEST_F (MetronomeTest, HighTempo)
{
  ...
  tempo_map_->add_tempo_event (
    0, 240.0, TempoMap::CurveType::Constant); // 240 BPM
