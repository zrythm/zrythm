...
#define GTK_FISHBOWL_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST ( \
    (klass), GTK_TYPE_FISHBOWL, GtkFishbowlClass))
#define GTK_IS_FISHBOWL(obj) \
  (G_TYPE_CHECK_INSTANCE_TYPE ( \
    (obj), GTK_TYPE_FISHBOWL))
#define GTK_IS_FISHBOWL_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_TYPE ( \
    (klass), GTK_TYPE_FISHBOWL))
#define GTK_FISHBOWL_GET_CLASS(obj) \
  (G_TYPE_INSTANCE_GET_CLASS ( \
    (obj), GTK_TYPE_FISHBOWL, GtkFishbowlClass))
...
{
  GtkFishbowlPrivate * priv =
    gtk_fishbowl_get_instance_private (fishbowl);

  gtk_widget_set_has_window (
    GTK_WIDGET (fishbowl), FALSE);
...
static void
gtk_fishbowl_get_preferred_width (
  GtkWidget * widget,
  int *       minimum,
  int *       natural)
{
  ...
  for (children = priv->children; children;
       children = children->next)
    {
      ...
      gtk_widget_get_preferred_width (
        child->widget, &child_min, &child_nat);
  ...
  *natural = 0;
  ...
  for (children = priv->children; children;
       children = children->next)
    {
      ...
      gtk_widget_get_preferred_width (
        child->widget, &min_width, NULL);
      gtk_widget_get_preferred_height_for_width (
        child->widget, min_width, &child_min,
        &child_nat);
  ...
  for (children = priv->children; children;
       children = children->next)
    {
      ...
      gtk_widget_get_preferred_size (
        child->widget, &child_requisition, NULL);
      child_allocation.x =
        allocation->x
        + round (
          child->x
  ...
  GtkWidget *    widget)
{
  ...
  gtk_widget_set_parent (
    widget, GTK_WIDGET (fishbowl));

  priv->children =
    g_list_prepend (priv->children, child_info);
  priv->count++;
  g_object_notify_by_pspec (
    G_OBJECT (fishbowl), props[PROP_COUNT]);
}
      ...
      if (child->widget == widget)
        {
          ...
          priv->children = g_list_remove_link (
          ...
          if (
            was_visible
            && gtk_widget_get_visible (widget_bowl))
...
gtk_fishbowl_dispose (GObject * object)
{
  ...
  G_OBJECT_CLASS (gtk_fishbowl_parent_class)
    ->dispose (object);
}
  ...
  GParamSpec *   pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_ANIMATING:
      gtk_fishbowl_set_animating (
        fishbowl, g_value_get_boolean (value));
      break;
    ...
    case PROP_COUNT:
      gtk_fishbowl_set_count (
        fishbowl, g_value_get_uint (value));
      break;

    case PROP_UPDATE_DELAY:
      gtk_fishbowl_set_update_delay (
        fishbowl, g_value_get_int64 (value));
      break;
  ...
  GParamSpec * pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_ANIMATING:
      g_value_set_boolean (
        value,
        gtk_fishbowl_get_animating (fishbowl));
      break;

    case PROP_BENCHMARK:
      g_value_set_boolean (
        value,
        gtk_fishbowl_get_benchmark (fishbowl));
      break;

    case PROP_COUNT:
      g_value_set_uint (
        value, gtk_fishbowl_get_count (fishbowl));
      break;

    case PROP_FRAMERATE:
      g_value_set_double (
        value,
        gtk_fishbowl_get_framerate (fishbowl));
      break;

    case PROP_UPDATE_DELAY:
      g_value_set_int64 (
        value,
        gtk_fishbowl_get_update_delay (fishbowl));
      break;
...
gtk_fishbowl_class_init (GtkFishbowlClass * klass)
{
  ...
  props[PROP_ANIMATING] = g_param_spec_boolean (
    "animating", "animating",
    "Whether children are moving around", FALSE,
    G_PARAM_READWRITE);
  ...
  props[PROP_COUNT] = g_param_spec_uint (
    "count", "Count", "Number of widgets", 0,
  ...
  g_object_class_install_properties (
    object_class, NUM_PROPERTIES, props);
}
  ...
  while (priv->count > count)
    {
      gtk_fishbowl_remove (
        ...
        ((GtkFishbowlChild *) priv->children->data)
          ->widget);
    }
  ...
  while (priv->count < count)
    {
      ...
      gtk_fishbowl_add (
        GTK_CONTAINER (fishbowl), new_widget);
    }
  ...
  gboolean      benchmark)
{
  ...
  g_object_notify_by_pspec (
    G_OBJECT (fishbowl), props[PROP_BENCHMARK]);
}
...
guess_refresh_interval (GdkFrameClock * frame_clock)
{
  ...
  for (i = gdk_frame_clock_get_history_start (
         frame_clock);
       i < gdk_frame_clock_get_frame_counter (
         frame_clock);
       i++)
    {
      ...
      t = gdk_frame_clock_get_timings (
        frame_clock, i);
      before = gdk_frame_clock_get_timings (
        frame_clock, i - 1);
      ...
      before_ts =
        gdk_frame_timings_get_frame_time (before);
      if (ts == 0 || before_ts == 0)
        continue;
...
gtk_fishbowl_do_update (GtkFishbowl * fishbowl)
{
  ...
  for (
    end = gdk_frame_clock_get_timings (
      frame_clock, end_counter);
  ...
  start_timestamp =
    gdk_frame_timings_get_presentation_time (start);
  end_timestamp =
    gdk_frame_timings_get_presentation_time (end);
  if (start_timestamp == 0 || end_timestamp == 0)
    {
      start_timestamp =
        gdk_frame_timings_get_frame_time (start);
      end_timestamp =
        gdk_frame_timings_get_frame_time (end);
    }

  n_frames = end_counter - start_counter;
  priv->framerate =
    ((double) n_frames) * G_USEC_PER_SEC
  ...
  interval =
    gdk_frame_timings_get_refresh_interval (end);
  if (interval == 0)
    {
      interval =
        guess_refresh_interval (frame_clock);
      if (interval == 0)
        return;
    }
  expected_frames = round (
    (double) (end_timestamp - start_timestamp)
    / interval);
  ...
  gpointer        unused)
{
  ...
  frame_time = gdk_frame_clock_get_frame_time (
  ...
  gboolean      animating)
{
  ...
  if (animating)
    {
      priv->tick_id = gtk_widget_add_tick_callback (
        GTK_WIDGET (fishbowl), gtk_fishbowl_tick,
        NULL, NULL);
    }
  else
    {
      ...
      gtk_widget_remove_tick_callback (
        GTK_WIDGET (fishbowl), priv->tick_id);
      priv->tick_id = 0;
      priv->framerate = 0;
      g_object_notify_by_pspec (
        G_OBJECT (fishbowl), props[PROP_FRAMERATE]);
  ...
  g_object_notify_by_pspec (
    G_OBJECT (fishbowl), props[PROP_ANIMATING]);
}
  ...
  gint64        update_delay)
{
  ...
  g_object_notify_by_pspec (
    G_OBJECT (fishbowl), props[PROP_UPDATE_DELAY]);
}
