...
test_fill_stereo_ports (void)
{
  ...
  g_assert_true (
    audio_frames_empty (&ports->l->buf[0], 20));
  g_assert_true (
    audio_frames_empty (&ports->r->buf[0], 20));
  for (int i = 0; i < 80; i++)
    {
      ...
      g_assert_true (math_floats_equal_epsilon (
        adj_clip_frame_l, ports->l->buf[20 + i],
        0.00001f));
      g_assert_true (math_floats_equal_epsilon (
        adj_clip_frame_r, ports->l->buf[20 + i],
        0.00001f));
    }
  ...
  for (int i = 0; i < 2; i++)
    {
      ...
      arranger_object_print (
        (ArrangerObject *) audio_track->lanes[0]
          ->regions[0]);
...
#define TEST_PREFIX "/audio/audio_region/"
  ...
  g_test_add_func (
    TEST_PREFIX
    "test load project with lower sample rate",
    (GTestFunc)
      test_load_project_with_different_sample_rate);
  g_test_add_func (
    TEST_PREFIX
    "test load project with selected audio region",
    (GTestFunc)
      test_load_project_with_selected_audio_region);
  g_test_add_func (
