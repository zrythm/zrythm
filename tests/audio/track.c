...
test_get_direct_folder_parent (void)
{
  ...
  Track * audio_group2 = track_create_empty_with_action (
    TRACK_TYPE_AUDIO_GROUP, NULL);
  ...
  Track * audio_group3 = track_create_empty_with_action (
    TRACK_TYPE_AUDIO_GROUP, NULL);
  ...
  track_select (
    ...
    F_NO_PUBLISH_EVENTS);
  tracklist_handle_move_or_copy (
    TRACKLIST, audio_group2, TRACK_WIDGET_HIGHLIGHT_INSIDE,
    GDK_ACTION_MOVE);
...
#define TEST_PREFIX "/audio/track/"
  ...
  g_test_add_func (
    TEST_PREFIX "test new track", (GTestFunc) test_new_track);
  g_test_add_func (
