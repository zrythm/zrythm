...
- Every code review (including triaging or applying an external review) must include an explicit comment pass: walk every comment and doc comment in the changed lines/new files and check them against the comment rules above (objective "what" statements, contract-only API docs, no caller coupling, plain language, concise). Fix violations directly and summarize what changed. Apply the same pass to my own changes before declaring work complete — do not wait to be asked
