...
project_init_flow_manager_append_callback (
  ...
  void *                   user_data)
{
  g_ptr_array_add (flow_mgr->callback_stack, (gpointer) cb);
  ...
  GError *                 error)
{
  ...
  ProjectInitDoneCallback cb = (ProjectInitDoneCallback) g_ptr_array_steal_index (
  ...
  if (ZRYTHM_HAVE_UI)
    {
      ...
      EVENTS_PUSH (EventType::ET_PROJECT_LOADED, self);
    }
}
...
get_newer_backup (Project * self)
{
  ...
  char * filepath =
    project_get_path (self, ProjectPath::PROJECT_PATH_PROJECT_FILE, false);
  ...
  GError **    error)
{
  ...
  self->last_selection = Project::SelectionType::Tracklist;
  ...
  snap_grid_init (
    self->snap_grid_timeline, SnapGridType::SNAP_GRID_TYPE_TIMELINE,
    NoteLength::NOTE_LENGTH_BAR, true);
  quantize_options_init (
    self->quantize_opts_timeline, NoteLength::NOTE_LENGTH_1_1);
  snap_grid_init (
    self->snap_grid_editor, SnapGridType::SNAP_GRID_TYPE_EDITOR,
    NoteLength::NOTE_LENGTH_1_8, true);
  quantize_options_init (
    self->quantize_opts_editor, NoteLength::NOTE_LENGTH_1_8);
  ...
  if (yaml_schema_ver > 3)
    {
      char * finished_file_path = project_get_path (
        PROJECT, ProjectPath::PROJECT_PATH_FINISHED_FILE, use_backup);
...
on_failed_to_init_pool:
  {
    ...
    g_signal_connect (
      err_win, "closed", G_CALLBACK (zrythm_exit_response_callback), NULL);
    return;
  }
    }
  engine_pre_setup (self->audio_engine);
    ...
    { \
      self->prefix##_selections = \
        (c_type##Selections *) arranger_selections_new ( \
          ArrangerSelectionsType::ARRANGER_SELECTIONS_TYPE_##type_name); \
    }
