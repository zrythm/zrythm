  ...
  for (size_t i = 0; i <= self->buffer_mask; ++i)
    {
      ...
      atomic_store_explicit (
        &self->buffer[i].sequence, i, std::memory_order_relaxed);
#else
      g_atomic_int_set (&self->buffer[i].sequence, (guint) i);
#endif
    }
#if MPMC_USE_STD_ATOMIC
  atomic_store_explicit (&self->enqueue_pos, 0, std::memory_order_relaxed);
  atomic_store_explicit (&self->dequeue_pos, 0, std::memory_order_relaxed);
#else
...
mpmc_queue_push_back (MPMCQueue * self, void * const data)
{
...
#if MPMC_USE_STD_ATOMIC
  size_t pos =
    atomic_load_explicit (&self->enqueue_pos, std::memory_order_relaxed);
#else
  ...
  for (;;)
    {
      ...
      unsigned int seq = (guint) atomic_load_explicit (
        &cell->sequence, std::memory_order_acquire);
#else
      ...
      if (dif == 0)
        {
          ...
          if (
            atomic_compare_exchange_weak_explicit (
              &self->enqueue_pos, &pos, pos + 1, std::memory_order_relaxed,
      ...
      else
        {
          ...
          pos = atomic_load_explicit (
            &self->enqueue_pos, std::memory_order_relaxed);
#else
  ...
  cell->data = data;
#if MPMC_USE_STD_ATOMIC
  atomic_store_explicit (&cell->sequence, pos + 1, std::memory_order_release);
#else
...
#if MPMC_USE_STD_ATOMIC
  size_t pos =
    atomic_load_explicit (&self->dequeue_pos, std::memory_order_relaxed);
#else
      ...
      if (dif == 0)
        {
          ...
          if (
            atomic_compare_exchange_weak_explicit (
              &self->dequeue_pos, &pos, (pos + 1), std::memory_order_relaxed,
              std::memory_order_relaxed))
#else
          if (g_atomic_int_compare_and_exchange (
...
#if MPMC_USE_STD_ATOMIC
          pos = atomic_load_explicit (
            &self->dequeue_pos, std::memory_order_relaxed);
#else
...
#if MPMC_USE_STD_ATOMIC
  atomic_store_explicit (
    &cell->sequence, pos + self->buffer_mask + 1, std::memory_order_release);
#else
