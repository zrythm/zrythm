...
void
TrackProcessor::fill_events_common (
  ...
  std::optional<std::pair<std::span<float>, std::span<float>>> stereo_ports) const
{
  ...
  std::visit (
    [&] (auto &&track) {
      ...
      if (!track->is_auditioner () && !TRANSPORT->isRolling ())
        {
          return;
        }
      ...
      const bool use_caches = !track->is_auditioner ();
...
// TODO
    ...
    convert_to_variant<ProcessableTrackPtrVariant> (get_track ()));
}
