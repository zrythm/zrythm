...
cached_plugin_descriptors_serialize_to_file (CachedPluginDescriptors * self)
{
  ...
  for (size_t i = 0; i < self->descriptors->len; i++)
    {
      const PluginDescriptor * descr = g_ptr_array_index (self->descriptors, i);
  ...
  for (size_t i = 0; i < self->blacklisted_sha1s->len; i++)
    {
      const char * str = g_ptr_array_index (self->blacklisted_sha1s, i);
  ...
  const char *              sha1)
{
  for (size_t i = 0; i < self->blacklisted_sha1s->len; i++)
    {
      const char * cur_sha1 = g_ptr_array_index (self->blacklisted_sha1s, i);
      ...
      for (size_t i = 0; i < self->descriptors->len; i++)
        {
          PluginDescriptor * cur_descr =
            g_ptr_array_index (self->descriptors, i);
          if (
  ...
  if (check_blacklisted)
    {
      for (size_t i = 0; i < self->blacklisted_sha1s->len; i++)
        {
          char * cur_sha1 = g_ptr_array_index (self->blacklisted_sha1s, i);
