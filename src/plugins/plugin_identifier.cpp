...
plugin_identifier_validate_slot_type_slot_combo (
  ...
  int             slot)
{
  return (slot_type == ZPluginSlotType::Z_PLUGIN_SLOT_INSTRUMENT && slot == -1)
         || (slot_type == ZPluginSlotType::Z_PLUGIN_SLOT_INVALID && slot == -1)
         || (slot_type != ZPluginSlotType::Z_PLUGIN_SLOT_INSTRUMENT && slot >= 0);
}
