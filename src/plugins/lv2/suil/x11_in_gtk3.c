...
GType
suil_x11_wrapper_get_type (
  void); // Accessor for SUIL_TYPE_X11_WRAPPER

#    define SUIL_TYPE_X11_WRAPPER \
      (suil_x11_wrapper_get_type ())
...
on_plug_removed (GtkSocket * sock, gpointer data)
{
  ...
  if (self->instance->handle)
    {
      self->instance->descriptor->cleanup (
...
suil_x11_wrapper_finalize (GObject * gobject)
{
  ...
  G_OBJECT_CLASS (suil_x11_wrapper_parent_class)
    ->finalize (gobject);
}
  ...
  if (gdk_event->any.window == window)
    {
      ...
      GtkWidget * widget =
        GTK_WIDGET (socket->instance->host_widget);
      GdkWindow * parent =
...
wrapper_wrap (SuilWrapper * wrapper, SuilInstance * instance)
{
  ...
  g_signal_connect (
    G_OBJECT (wrap), "plug-removed",
    G_CALLBACK (on_plug_removed), NULL);

  g_signal_connect (
    G_OBJECT (wrap), "size-allocate",
    G_CALLBACK (suil_x11_on_size_allocate), NULL);
