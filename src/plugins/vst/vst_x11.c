...
static void
dispatch_x_events (XEvent* event, VstPlugin* vstfx)
{
    ...
    case ConfigureNotify:
    {
      ...
      if (window == (Window) (vstfx->linux_window)) {
        ...
        if (width != vstfx->width || height != vstfx->height) {
          vstfx->width = width;
          vstfx->height = height;
          /*VstPlugin * plug = (VstPlugin *)(vstfx->plugin->ptr1);*/
        ...
        if (vstfx->linux_plugin_ui_window) {
          XMoveWindow (LXVST_XDisplay, (Window) vstfx->linux_plugin_ui_window, 0, 0);
        }
      }
  ...
  void* ptr)
{
  ...
  while (!gui_quit)
  {
    ...
    const gint64 elapsed_time_ms = (clock2 - clock1) / 1000;
      ...
      for (vstfx = vstfx_first; vstfx; vstfx = vstfx->next)
      {
        ...
        if (vstfx->destroy) {
          if (vstfx->linux_window) {
            vstfx->aeffect->dispatcher (vstfx->aeffect, effEditClose, 0, 0, NULL, 0.0);

            XDestroyWindow (LXVST_XDisplay, (Window) vstfx->linux_window);
        ...
        if (vstfx->dispatcher_wantcall) {
          vstfx->dispatcher_retval = vstfx->aeffect->dispatcher (
            vstfx->aeffect,
...
int vstfx_init (void* ptr)
{
  ...
  if(LXVST_XDisplay==NULL)
  {
    g_critical ("** ERROR ** VSTFX: Failed opening connection to X");
  ...
  if(thread_create_result!=0)
  {
    ...
    g_critical ("** ERROR ** VSTFX: Failed starting GUI event thread");
...
/**
 * Creates an editor for the plugin - normally called
 * from within the gui event loop after run_editor
  ...
  VstPlugin* vstfx)
{
  ...
  if (!(vstfx->aeffect->flags & effFlagsHasEditor))
  {
    g_critical ("** ERROR ** VSTFX: Plugin \"%s\" has no editor", vstfx->plugin->descr->name);
    return -1;
  }
  ...
  VstPlugin* vstfx)
{
  ...
  parent_window = (Window) vstfx->linux_window;
  ...
  vstfx->aeffect->dispatcher (vstfx->aeffect, effEditOpen, 0, (long)LXVST_XDisplay, (void*)(parent_window), 0 );
  ...
  vstfx->aeffect->dispatcher (vstfx->aeffect, effEditGetRect, 0, 0, &er, 0 );
...
/**
 * Remove a VST plugin instance from the linked list
