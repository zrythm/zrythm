...
chord_preset_pack_contains_name (const ChordPresetPack * self, const char * name)
{
  for (size_t i = 0; i < self->presets->len; i++)
    {
      ChordPreset * pset = (ChordPreset *) g_ptr_array_index (self->presets, i);
  ...
  for (size_t i = 0; i < self->presets->len; i++)
    {
      ChordPreset * cur_pset =
        (ChordPreset *) g_ptr_array_index (self->presets, i);
      if (pset == cur_pset)
        return true;
    }
...
chord_preset_pack_delete_preset (ChordPresetPack * self, ChordPreset * pset)
{
  ...
  EVENTS_PUSH (EventType::ET_CHORD_PRESET_REMOVED, NULL);
}
...
chord_preset_pack_set_name (ChordPresetPack * self, const char * name)
{
  ...
  EVENTS_PUSH (EventType::ET_CHORD_PRESET_PACK_EDITED, NULL);
}
...
chord_preset_pack_clone (const ChordPresetPack * src)
{
  ...
  for (size_t i = 0; i < src->presets->len; i++)
    {
      const ChordPreset * src_pset =
        (ChordPreset *) g_ptr_array_index (src->presets, i);
