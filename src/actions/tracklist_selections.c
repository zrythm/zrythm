...
{
  for (int i = 0;
       i < self->foldable_tls_before->num_tracks;
       i++)
    {
      Track * own_tr =
  ...
  GError **                      error)
{
  UNDO_MANAGER_PERFORM_AND_PROPAGATE_ERR (
    ...
    track_pos, lane_pos, pos, num_tracks,
    edit_type, ival_after, color_new, val_before,
