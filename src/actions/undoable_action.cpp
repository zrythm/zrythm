...
#define INIT_LOADED(uc, sc, cc) \
  case UndoableActionType::UA_##uc: \
...
need_transport_total_bar_update (UndoableAction * self, bool _do)
{
  switch (self->type)
    {
    case UndoableActionType::UA_ARRANGER_SELECTIONS:
      {
        ArrangerSelectionsAction * action = (ArrangerSelectionsAction *) self;
        if (
          (action->type == ArrangerSelectionsActionType::AS_ACTION_CREATE && _do)
          || (action->type == ArrangerSelectionsActionType::AS_ACTION_DELETE && !_do)
          || (action->type == ArrangerSelectionsActionType::AS_ACTION_DUPLICATE && _do)
          || (action->type == ArrangerSelectionsActionType::AS_ACTION_LINK && _do))
          {
            return false;
          }
      }
      break;
    case UndoableActionType::UA_TRACKLIST_SELECTIONS:
      {
        ...
        if (
          ...
          {
            if (
              action->edit_type == EditTrackActionType::EDIT_TRACK_ACTION_TYPE_MUTE
              || action->edit_type == EditTrackActionType::EDIT_TRACK_ACTION_TYPE_SOLO
              || action->edit_type
                   == EditTrackActionType::EDIT_TRACK_ACTION_TYPE_LISTEN
              || action->edit_type
                   == EditTrackActionType::EDIT_TRACK_ACTION_TYPE_VOLUME
              || action->edit_type
                   == EditTrackActionType::EDIT_TRACK_ACTION_TYPE_PAN)
              {
                return false;
              }
          }
...
undoable_action_get_plugins (UndoableAction * self, GPtrArray * arr)
{
  switch (self->type)
    ...
    case UndoableActionType::UA_TRACKLIST_SELECTIONS:
      ...
      break;
    case UndoableActionType::UA_MIXER_SELECTIONS:
      {
...
#define STRINGIZE_UA(caps, cc, sc) \
  case UndoableActionType::UA_##caps: \
