  ...
  GError **               error)
{
  ...
  yyjson_mut_obj_add_int (
    doc, at_obj, "automationMode", (int64_t) at->automation_mode);
  yyjson_mut_obj_add_int (doc, at_obj, "recordMode", (int64_t) at->record_mode);
  ...
  GError **        error)
{
  ...
  yyjson_mut_obj_add_int (doc, track_obj, "type", (int64_t) track->type);
  ...
  if (track->type == TrackType::TRACK_TYPE_MARKER)
    {
  ...
  yyjson_mut_obj_add_int (
    doc, track_obj, "inSignalType", (int64_t) track->in_signal_type);
  yyjson_mut_obj_add_int (
    doc, track_obj, "outSignalType", (int64_t) track->out_signal_type);
  ...
  GError **    error)
{
  ...
  if (track->lanes_size > 0)
    {
      track->lanes = object_new_n (track->lanes_size, TrackLane *);
  ...
  if (track->type == TrackType::TRACK_TYPE_CHORD)
    {
      ...
      if (track->scales_size > 0)
        {
          track->scales = object_new_n (track->scales_size, ScaleObject *);
  ...
  if (track->type == TrackType::TRACK_TYPE_MARKER)
    {
      ...
      if (track->markers_size > 0)
        {
          track->markers = object_new_n (track->markers_size, Marker *);
  ...
  if (modulators_arr)
    {
      ...
      if (track->modulators_size > 0)
        {
          track->modulators = object_new_n (track->modulators_size, Plugin *);
  ...
  if (track->automation_tracklist.ats_size > 0)
    {
      track->automation_tracklist.ats =
        object_new_n (track->automation_tracklist.ats_size, AutomationTrack *);
  ...
  if (track->children_size > 0)
    {
      track->children = object_new_n (track->children_size, unsigned int);
