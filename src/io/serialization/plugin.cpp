...
plugin_identifier_serialize_to_json (
  ...
  GError **                error)
{
  yyjson_mut_obj_add_int (doc, pi_obj, "slotType", (int64_t) pi->slot_type);
  ...
  GError **                error)
{
  ...
  yyjson_mut_obj_add_int (doc, pd_obj, "category", (int64_t) pd->category);
  if (pd->category_str)
    {
  ...
  yyjson_mut_obj_add_int (doc, pd_obj, "architecture", (int64_t) pd->arch);
  yyjson_mut_obj_add_int (doc, pd_obj, "protocol", (int64_t) pd->protocol);
  if (pd->path)
    {
  ...
  yyjson_mut_obj_add_int (
    doc, pd_obj, "minBridgeMode", (int64_t) pd->min_bridge_mode);
  ...
  GError **             error)
{
  ...
  yyjson_mut_obj_add_int (doc, ps_obj, "bridgeMode", (int64_t) ps->bridge_mode);
  return true;
}
  ...
  for (size_t i = 0; i < col->descriptors->len; i++)
    {
      const PluginDescriptor * descr =
        (PluginDescriptor *) g_ptr_array_index (col->descriptors, i);
  ...
  GError **    error)
{
  ...
  if (bank->presets_size > 0)
    {
      bank->presets = object_new_n (bank->presets_size, PluginPreset *);
  ...
  GError **    error)
{
  ...
  if (plugin->in_ports_size > 0)
    {
      plugin->in_ports = object_new_n (plugin->in_ports_size, Port *);
  ...
  if (plugin->out_ports_size > 0)
    {
      plugin->out_ports = object_new_n (plugin->out_ports_size, Port *);
  ...
  if (plugin->banks_size > 0)
    {
      plugin->banks = object_new_n (plugin->banks_size, PluginBank *);
