...
undoable_action_serialize_to_json (
  ...
  GError **              error)
{
  yyjson_mut_obj_add_int (doc, action_obj, "type", (int64_t) action->type);
  ...
  GError **                        error)
{
  ...
  yyjson_mut_obj_add_int (
    doc, action_obj, "editType", (int64_t) action->edit_type);
  yyjson_mut_obj_add_int (
    doc, action_obj, "resizeType", (int64_t) action->resize_type);
  ...
  GError **                     error)
{
  ...
  yyjson_mut_obj_add_int (doc, action_obj, "type", (int64_t) action->type);
  yyjson_mut_obj_add_int (
    doc, action_obj, "slotType", (int64_t) action->slot_type);
  ...
  yyjson_mut_obj_add_int (
    doc, action_obj, "newBridgeMode", (int64_t) action->new_bridge_mode);
  if (action->setting)
    {
  ...
  GError **                         error)
{
  ...
  yyjson_mut_obj_add_int (doc, action_obj, "type", (int64_t) action->type);
  yyjson_mut_obj_add_int (
    doc, action_obj, "trackType", (int64_t) action->track_type);
  ...
  GError **                   error)
{
  ...
  if (out_tracks_arr)
    {
      ...
      if (action->num_out_tracks > 0)
        {
          action->out_tracks =
            object_new_n ((size_t) action->num_out_tracks, unsigned int);
  ...
  if (src_sends_arr)
    {
      ...
      if (action->src_sends_size > 0)
        {
          action->src_sends =
            object_new_n (action->src_sends_size, ChannelSend *);
  ...
  action->edit_type = (EditTrackActionType) yyjson_get_int (
    yyjson_obj_iter_get (&it, "editType"));
  ...
  if (ival_before_arr)
    {
      if (action->num_tracks > 0)
        {
          action->ival_before = object_new_n ((size_t) action->num_tracks, int);
      ...
      if (action->num_tracks > 0)
        {
          action->colors_before =
            object_new_n ((size_t) action->num_tracks, GdkRGBA);
