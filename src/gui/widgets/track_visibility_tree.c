  ...
  TrackVisibilityTreeWidget * self)
{
  ...
  gtk_tree_model_get (
    model, &iter, COL_CHECKBOX, &fixed, -1);

  /* get track */
  Track * track;
  gtk_tree_model_get (
    model, &iter, COL_TRACK, &track, -1);
  ...
  /* set new value */
  gtk_list_store_set (
    GTK_LIST_STORE (model), &iter, COL_CHECKBOX,
    fixed, -1);
  ...
  GtkTreeView *               tree_view)
{
  ...
  column = gtk_tree_view_column_new_with_attributes (
    _ ("Track Name"), renderer, "text", COL_NAME,
    NULL);
  gtk_tree_view_append_column (
    GTK_TREE_VIEW (tree_view), column);
}
...
{
  TrackVisibilityTreeWidget * self = g_object_new (
