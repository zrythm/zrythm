...
#define DEFINE_DRAG_END_POS_SET( \
  ...
  { \
    ...
    if (position_is_equal ( \
          &TL_SELECTIONS->sc##s[0]->pos_name, &new_pos)) \
      return; \
    UndoableAction * ua = (UndoableAction *) \
      edit_timeline_selections_action_new ( \
        TL_SELECTIONS, ETS_##caps##_##pos_name_caps, 0, \
        NULL, &new_pos); \
...
#define DEFINE_DRAG_END_POS_RESIZE_L(cc, sc) \
  ...
  { \
    ...
    timeline_selections_free (tl_clone); \
    if (position_is_equal ( \
          &TL_SELECTIONS->sc##s[0]->start_pos, &new_pos)) \
      return; \
    long ticks_diff = \
...
#define DEFINE_DRAG_END_POS_RESIZE_R(cc, sc) \
  ...
  { \
    ...
    timeline_selections_free (tl_clone); \
    if (position_is_equal ( \
          &TL_SELECTIONS->sc##s[0]->end_pos, &new_pos)) \
      return; \
    long ticks_diff = \
      position_to_ticks (&new_pos) \
...
on_drag_end ()
{
  Position start_pos;
  timeline_selections_get_start_pos (
    TL_SELECTIONS, &start_pos, F_NO_TRANSIENTS);
  long ticks_diff =
    position_to_ticks (&start_pos)
    - position_to_ticks (&earliest_start_pos);
  /* remove the diff since it will get added
   * in the moving action */
  timeline_selections_add_ticks (
    TL_SELECTIONS, -ticks_diff, 0, AO_UPDATE_ALL);
  UndoableAction * ua =
    (UndoableAction *) move_timeline_selections_action_new (
  ...
  if (position_is_after_or_equal (pos, START_POS))
    {
      timeline_selections_add_ticks (
        TL_SELECTIONS,
        position_to_ticks (pos) - position_to_ticks (obj), 0,
        AO_UPDATE_ALL);
...
#define ADD_WIDGET(widget) \
      ...
      if (Z_IS_REGION_WIDGET (fo))
        {
          ...
          ZRegion * r =
            region_get_main_trans_region (rw_prv->region);
      ...
      else if (Z_IS_CHORD_OBJECT_WIDGET (fo))
        {
          ...
          dm = digital_meter_widget_new_for_position (
            ...
            chord_object_pos_setter, on_drag_end,
            _ ("position"));
          digital_meter_set_draw_line (dm, 1);
          ADD_WIDGET (dm);
      ...
      else if (Z_IS_SCALE_OBJECT_WIDGET (fo))
        {
          ...
          dm = digital_meter_widget_new_for_position (
            ...
            scale_object_pos_setter, on_drag_end,
            _ ("position"));
          digital_meter_set_draw_line (dm, 1);
          ADD_WIDGET (dm);
      ...
      else if (Z_IS_MARKER_WIDGET (fo))
        {
          ...
          DigitalMeterWidget * dm;
          dm = digital_meter_widget_new_for_position (
            marker, on_drag_begin, marker_get_pos,
            marker_pos_setter, on_drag_end, _ ("position"));
          digital_meter_set_draw_line (dm, 1);
          ADD_WIDGET (dm);
   ...
   * for moving */
  else
    {
      ...
      DigitalMeterWidget * dm;
      dm = digital_meter_widget_new_for_position (
        pos, on_drag_begin, get_pos, set_pos, on_drag_end,
        _ ("position"));
      digital_meter_set_draw_line (dm, 1);
      ADD_WIDGET (dm);
