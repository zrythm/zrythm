...
gtk_flipper_size_allocate (GtkWidget * widget, int width, int height, int baseline)
{
  ...
  if (self->flip_horizontal)
    {
      transform = gsk_transform_translate (
        transform, &Z_GRAPHENE_POINT_INIT ((float) width, 0.f));
  ...
  if (self->flip_vertical)
    {
      transform = gsk_transform_translate (
        transform, &Z_GRAPHENE_POINT_INIT (0.f, (float) height));
      transform = gsk_transform_scale (transform, 1, -1);
      baseline = -1;
    }
  ...
  GParamSpec *   pspec)
{
  ...
  switch (property_id)
    {
    case PROP_CHILD:
      gtk_flipper_set_child (self, GTK_WIDGET (g_value_get_object (value)));
      break;
...
gtk_flipper_class_init (GtkFlipperClass * klass)
{
  ...
  properties[PROP_CHILD] = g_param_spec_object (
    ...
    (GParamFlags) ((int) G_PARAM_READWRITE | (int) G_PARAM_EXPLICIT_NOTIFY
                   | (int) G_PARAM_STATIC_STRINGS));

  /**
   * GtkFlipper:flip-horizontal: (attributes
   * org.gtk.Property.get=gtk_flipper_get_flip_horizontal
  ...
  /**
   * GtkFlipper:flip-vertical: (attributes
   * org.gtk.Property.get=gtk_flipper_get_flip_vertical
...
/**
 * gtk_flipper_get_flip_horizontal: (attributes
 * org.gtk.Method.get_property=flip-horizontal)
 * @self: a `GtkFlipper`
 *
...
/**
 * gtk_flipper_set_flip_horizontal: (attributes
 * org.gtk.Method.set_property=flip-horizontal)
 * @self: a `GtkFlipper`
...
/**
 * gtk_flipper_get_flip_vertical: (attributes
 * org.gtk.Method.get_property=flip-vertical)
 * @self: a `GtkFlipper`
 *
...
/**
 * gtk_flipper_set_flip_vertical: (attributes
 * org.gtk.Method.set_property=flip-vertical)
 * @self: a `GtkFlipper`
