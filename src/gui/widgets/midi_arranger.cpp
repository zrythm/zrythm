  ...
  Region *         region)
{
  ...
  MidiNote * midi_note = midi_note_new (
    &region->id, &local_pos, &local_pos, (midi_byte_t) note, velocity);
  ArrangerObject * midi_note_obj = (ArrangerObject *) midi_note;
  ...
  /* set end pos */
  Position tmp;
  position_set_min_size (&midi_note_obj->pos, &tmp, self->snap_grid);
  ...
  EVENTS_PUSH (EventType::ET_ARRANGER_OBJECT_CREATED, midi_note);

  /* select it */
  arranger_object_select (
    midi_note_obj, F_SELECT, autofilling ? F_APPEND : F_NO_APPEND,
    F_NO_PUBLISH_EVENTS);
  ...
  bool             dry_run)
{
   ...
   * pos */
  double delta = pos->ticks - (self->start_object->pos.ticks + r_obj->pos.ticks);
  ...
  for (int i = 0; i < MA_SELECTIONS->num_midi_notes; i++)
    {
       ...
       * snap it */
      position_set_to_pos (&new_global_start_pos, &new_start_pos);
      ...
      if (
        ...
        {
          position_snap (
            &self->earliest_obj_start_pos, &new_global_start_pos, NULL,
            clip_editor_region, self->snap_grid);
        }

      /* convert it back to a local pos */
      position_set_to_pos (&new_start_pos, &new_global_start_pos);
      position_add_ticks (&new_start_pos, -r_obj->pos.ticks);

      if (
        !position_is_positive (&new_global_start_pos)
        || position_is_after_or_equal (&new_start_pos, &mn_obj->end_pos))
        {
          return -1;
        }
      else if (!dry_run)
  ...
  bool             dry_run)
{
   ...
   * pos */
  double delta =
    pos->ticks - (self->start_object->end_pos.ticks + r_obj->pos.ticks);
  g_debug ("delta %f", delta);
  ...
  for (int i = 0; i < MA_SELECTIONS->num_midi_notes; i++)
    {
      ...
      position_set_to_pos (&new_end_pos, &midi_note->base.end_pos);
      ...
      position_add_ticks (&new_global_end_pos, r_obj->pos.ticks);
      ...
      if (position_is_before_or_equal (&new_end_pos, &mn_obj->pos))
        return -1;
      else if (!dry_run)
        {
          arranger_object_end_pos_setter (mn_obj, &new_end_pos);
        }
    }
...
midi_arranger_listen_notes (ArrangerWidget * self, bool listen)
{
  ...
  Position                 start_pos;
  arranger_selections_get_start_pos (sel, &start_pos, F_NOT_GLOBAL);
  double ticks_cutoff = start_pos.ticks + (double) TRANSPORT->ticks_per_beat;
  ...
  double           y)
{
  ...
  ArrangerObject * mn_obj = arranger_widget_get_hit_arranger_object (
    self, ArrangerObjectType::ARRANGER_OBJECT_TYPE_MIDI_NOTE, x, y);
...
do_vertical_zoom (ArrangerWidget * self, double hover_y, double delta_y)
{
  ...
  double size_before = gtk_widget_get_height (GTK_WIDGET (MW_PIANO_ROLL_KEYS));
  if (hover_y < 0)
    {
      hover_y = adj_val;
    }
  ...
  piano_roll_set_notes_zoom (
    PIANO_ROLL, PIANO_ROLL->notes_zoom * (float) multiplier, 0);
  size_after = size_before * multiplier;

  if (math_floats_equal (PIANO_ROLL->notes_zoom, notes_zoom_before))
    {
  ...
  editor_settings_set_scroll_start_y (
    settings, (int) (adj_perc * size_after - diff), F_NO_VALIDATE);
}
