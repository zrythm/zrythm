  ...
  if (pan_val < 0.5f)
    {
      gtk_snapshot_append_color (
        snapshot, &color,
        &GRAPHENE_RECT_INIT (
          value_px, 0, half_width - value_px,
          height));
    }
  else
    {
      gtk_snapshot_append_color (
        snapshot, &color,
        &GRAPHENE_RECT_INIT (
          half_width, 0, value_px - half_width,
          height));
    }
  ...
  BalanceControlWidget * self)
{
  ...
  float new_val =
    CLAMP (GET_VAL + (float) diff, 0.0f, 1.0f);
