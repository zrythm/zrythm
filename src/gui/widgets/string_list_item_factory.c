...
{
  GtkLabel * lbl =
    GTK_LABEL (gtk_list_item_get_child (list_item));
  GObject *    gobj = gtk_list_item_get_item (list_item);
  const char * str = "(invalid)";
  if (GTK_IS_STRING_OBJECT (gobj))
    {
      GtkStringObject * sobj = GTK_STRING_OBJECT (
  ...
  else if (ADW_IS_ENUM_LIST_ITEM (gobj))
    {
      ...
      StringListItemFactoryEnumStringGetter getter = user_data;
  ...
  StringListItemFactoryEnumStringGetter getter)
{
  ...
  g_signal_connect (
    factory, "setup", G_CALLBACK (setup_cb), NULL);
  g_signal_connect (
    factory, "bind", G_CALLBACK (bind_cb), getter);
  g_signal_connect (
    factory, "teardown", G_CALLBACK (teardown_cb), NULL);
