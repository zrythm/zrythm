...
set_menu_model (GtkMenuButton * menu_btn)
{
  GMenuModel * menu_model =
    G_MENU_MODEL (tracklist_widget_generate_add_track_menu ());
...
{
  return g_object_new (ADD_TRACK_MENU_BUTTON_WIDGET_TYPE, NULL);
}
...
static void
add_track_menu_button_widget_init (AddTrackMenuButtonWidget * self)
{
  ...
  gtk_widget_add_css_class (GTK_WIDGET (self), "add-track-menu-button");
  gtk_widget_set_tooltip_text (GTK_WIDGET (self->menu_btn), _ ("Add track"));
  gtk_menu_button_set_icon_name (self->menu_btn, "add");
  gtk_widget_set_parent (GTK_WIDGET (self->menu_btn), GTK_WIDGET (self));
}

static void
add_track_menu_button_widget_class_init (AddTrackMenuButtonWidgetClass * _klass)
{
