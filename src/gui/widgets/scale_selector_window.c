  ...
  for (int i = 0; i < 12; i++)
    {
      ...
      clone = musical_scale_new (self->descr->type, i);
  ...
  for (MusicalScaleType i = SCALE_CHROMATIC;
       i <= SCALE_JAPANESE2; i++)
    {
      if (self->creator_types[i] != child)
        continue;
      clone = musical_scale_new (i, self->descr->root_key);
      musical_scale_free (self->descr);
      self->descr = clone;
      break;
    }
...
#define SELECT_CHILD(flowbox, child) \
  ...
  gtk_flow_box_select_child (
    ...
    GTK_FLOW_BOX_CHILD (self->creator_types[descr->type]));
  ...
  ScaleSelectorWindowWidget * self)
{
  ...
  for (MusicalScaleType i = SCALE_CHROMATIC;
       i <= SCALE_JAPANESE2; i++)
    {
      ...
      const char * str = musical_scale_type_to_string (i);
