  ...
  FolderChannelWidget * self)
{
  ...
  if (x < w / 2)
    {
      if (this->pos <= MW_MIXER->start_drag_track->pos)
        pos = this->pos;
      else
        {
          Track * prev = tracklist_get_prev_visible_track (TRACKLIST, this);
  ...
  else
    {
      if (this->pos >= MW_MIXER->start_drag_track->pos)
        ...
        {
          Track * next = tracklist_get_next_visible_track (TRACKLIST, this);
...
{
  FolderChannelWidget * self = g_object_new (FOLDER_CHANNEL_WIDGET_TYPE, NULL);
  self->track = track;
