...
draw_name (TrackCanvasWidget * self, GtkSnapshot * snapshot, int width)
{
  ...
  gtk_snapshot_translate (snapshot, &Z_GRAPHENE_POINT_INIT (22.f, 2.f));
      ...
      for (int j = 0; j < lane->num_buttons; j++)
        {
          ...
          if (cb == tw->clicked_button)
            ...
            {
              state =
                CustomButtonWidgetState::CUSTOM_BUTTON_WIDGET_STATE_HOVERED;
            }
      ...
      if (at->num_top_left_buttons == 0)
        {
          ...
          cb->owner_type =
            CustomButtonWidgetOwner::CUSTOM_BUTTON_WIDGET_OWNER_AT;
          cb->owner = at;
          /*char text[500];*/
          /*sprintf (*/
          /*text, "%d - %s",*/
          ...
          for (int j = 0; j < at->num_bot_right_buttons; j++)
            {
              ...
              state = CustomButtonWidgetState::CUSTOM_BUTTON_WIDGET_STATE_NORMAL;

              if (cb == tw->clicked_button)
                {
                  /* currently clicked button */
                  state =
                    CustomButtonWidgetState::CUSTOM_BUTTON_WIDGET_STATE_ACTIVE;
                }
              else if (hovered_cb == cb)
                {
                  state =
