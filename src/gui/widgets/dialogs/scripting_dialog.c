  ...
  ScriptingDialogWidget * self)
{
  ...
  char * script_content = gtk_text_buffer_get_text (
    GTK_TEXT_BUFFER (self->buffer), &start_iter, &end_iter,
    false);

  GuileScriptLanguage lang = (GuileScriptLanguage)
    adw_combo_row_get_selected (self->lang_select_combo_row);
  ...
  void *       data)
{
  ...
  GuileScriptLanguage script_lang = (GuileScriptLanguage)
...
scripting_dialog_widget_new ()
{
  ScriptingDialogWidget * self = g_object_new (
    ...
    _ ("Scripting Interface"), "icon-name", "zrythm", NULL);
...
scripting_dialog_widget_init (ScriptingDialogWidget * self)
{
      ...
      if (!g_file_test (full_path, G_FILE_TEST_EXISTS))
        {
          g_free (full_path);
          full_path =
            g_build_filename (dir, "print-all-tracks", NULL);
  ...
  if (!ret)
    {
      g_critical (
        "error reading <%s> contents: %s", full_path,
        err->message);
      return;
    }
  ...
  gtk_text_buffer_set_text (
    GTK_TEXT_BUFFER (self->buffer), script_contents, -1);
  ...
  self->execute_btn =
    GTK_BUTTON (gtk_button_new_with_label (_ ("Execute")));
  ...
  gtk_widget_set_hexpand (GTK_WIDGET (scroll), true);
  gtk_widget_set_vexpand (GTK_WIDGET (scroll), true);
  self->output = GTK_LABEL (
    gtk_label_new (_ ("The result will be printed here")));
