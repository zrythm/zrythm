  ...
  PortConnectionRowWidget * self)
{
  ...
  if (!ret)
    {
      HANDLE_ERROR (
        err, "%s", _ ("Failed to enable connection"));
    }
  ...
  bool                           is_input)
{
  ...
  /* power button */
  GtkToggleButton * btn =
    z_gtk_toggle_button_new_with_icon ("network-connect");
  ...
  g_signal_connect (
    G_OBJECT (btn), "toggled", G_CALLBACK (on_enable_toggled),
    self);
  ...
  if (!IS_PORT_AND_NONNULL (port))
    {
      g_critical (
        "failed to find port for '%s'", port_id->label);
      return NULL;
    }
  ...
  gtk_overlay_set_child (
    GTK_OVERLAY (self->overlay), GTK_WIDGET (self->slider));
