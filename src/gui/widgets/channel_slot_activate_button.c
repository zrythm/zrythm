...
on_toggled (
  ...
  gpointer user_data)
{
  ChannelSlotActivateButtonWidget * self = Z_CHANNEL_SLOT_ACTIVATE_BUTTON_WIDGET (user_data);

  Plugin * pl = channel_slot_widget_get_plugin (self->owner);
  if (pl)
    {
      plugin_set_enabled (pl, gtk_toggle_button_get_active (btn), F_PUBLISH_EVENTS);
    }
}
...
channel_slot_activate_button_widget_new (ChannelSlotWidget * owner)
{
  ...
  self->toggled_id = g_signal_connect (
    G_OBJECT (self), "toggled", G_CALLBACK (on_toggled),
    self);
...
static void
channel_slot_activate_button_widget_class_init (ChannelSlotActivateButtonWidgetClass * klass)
{
