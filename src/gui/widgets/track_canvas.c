  ...
  int                 h)
{
  ...
  if (PANGO_IS_LAYOUT (self->automation_value_layout))
  ...
  for (int i = 0; i < num_buttons; i++)
    {
      ...
      if (top)
        {
          cb->x =
            width - (TRACK_BUTTON_SIZE + TRACK_BUTTON_PADDING) *
            (num_buttons - i);
  ...
  for (int i = 0; i < atl->num_ats; i++)
    {
      ...
      float orig_start_x =
        (float)
        (cb->x + cb->width + TRACK_BUTTON_PADDING);
      float orig_start_y =
        (float)
        total_height +
          TRACK_BUTTON_PADDING_FROM_EDGE;
      float remaining_x =
        (float)
        (width - orig_start_x) - pangorect.width;
      float remaining_y =
  ...
  GtkSnapshot * snapshot)
{
  ...
  draw_color_area (
    self, snapshot, &rect, height);

  draw_name (self, snapshot, width);

  draw_buttons (
    self, snapshot, 1, width);
  ...
  TrackCanvasWidget * self)
{
  ...
  G_OBJECT_CLASS (
    track_canvas_widget_parent_class)->
      finalize (G_OBJECT (self));
}

static void
track_canvas_widget_init (
  TrackCanvasWidget * self)
{
}
