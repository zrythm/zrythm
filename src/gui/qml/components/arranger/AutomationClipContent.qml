...
Item {
  ...
  function generateAutomationCurveImpl() {
    if (!automationClip || contentWidth <= 0) {
      automationValues = [];
    ...
    automationValues = QmlUtils.getAutomationClipValues(automationClip, contentWidth);
    automationCanvas.requestPaint();
  }
  ...
  // Generate automation curve when clip or size changes
  onAutomationClipChanged: generateAutomationCurve()
  ...
  Connections {
    ...
    target: root.automationClip.automationPoints
  }
  ...
  Canvas {
    ...
    onPaint: {
      ...
      const regionColor = ZrythmTheme.clipContentColor;
      ...
      ctx.fillStyle = regionColor;
      const automationPoints = root.automationClip.automationPoints;
      for (let i = 0; i < automationPoints.length; i++) {
        ...
        if (ap) {
          ...
          const regionPos = root.automationClip.position.ticks;
          const regionLength = root.automationClip.timelineLengthTicks;
