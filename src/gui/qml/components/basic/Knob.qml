...
Control {
  ...
  MouseArea {
    ...
    onPositionChanged: function (mouse) {
      if (root.dragging) {
        if (root.expectWarpEvent) {
          root.expectWarpEvent = false;

          // Ignore the synthetic move event generated by our own cursor warp (it lands back on the drag start position)
          if (Math.abs(mouse.x - root.lastCoordinates.x) <= 1 && Math.abs(mouse.y - root.lastCoordinates.y) <= 1) {
            root.warpsSinceLastEcho = 0;
          ...
          root.warpsSinceLastEcho += 1;
          if (root.warpsSinceLastEcho >= 5) {
            root.warpingEffective = false;
          }
        }

        // Accumulate incremental movement so the drag behaves the same whether or not cursor warping is honored by the platform
        root.accumulatedDragDeltaX += mouse.x - root.lastMousePos.x;
        root.accumulatedDragDeltaY += root.lastMousePos.y - mouse.y; // Invert Y for natural dragging
        root.lastMousePos = Qt.point(mouse.x, mouse.y);

        // Lock the drag axis once movement exceeds a small threshold
        if (root.dragAxis === 0 && Math.abs(root.accumulatedDragDeltaX) + Math.abs(root.accumulatedDragDeltaY) > 3) {
          root.dragAxis = Math.abs(root.accumulatedDragDeltaY) >= Math.abs(root.accumulatedDragDeltaX) ? 2 : 1;
        }

        const delta = root.dragAxis === 0 ? 0 : (root.dragAxis === 1 ? root.accumulatedDragDeltaX : root.accumulatedDragDeltaY);

        const sensitivity = 0.004;
        const newValue = Math.max(root.minValue, Math.min(root.maxValue, root.valueAtStart + delta * sensitivity));

        root.value = newValue;
        knobCanvas.requestPaint();

        if (root.warpingEffective) {
          // Recompute the warp target per event so it tracks the knob if it moves on screen mid-drag
          CursorManager.warpCursor(root.mapToGlobal(root.lastCoordinates.x, root.lastCoordinates.y));
          root.expectWarpEvent = true;
        }
      }
    }
    onPressed: function (mouse) {
      if (mouse.button === Qt.LeftButton) {
        ...
        root.lastMousePos = Qt.point(mouse.x, mouse.y);
        root.accumulatedDragDeltaX = 0;
        root.accumulatedDragDeltaY = 0;
        root.dragAxis = 0;
        root.expectWarpEvent = false;
        root.warpingEffective = true;
        root.warpsSinceLastEcho = 0;
