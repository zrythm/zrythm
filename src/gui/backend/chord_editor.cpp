  ...
  for (int i = 0; i < 12; i++)
    {
      self->chords[i] = chord_descriptor_new (
        (MusicalNote) ((int) MusicalNote::NOTE_C + i), 1,
        (MusicalNote) ((int) MusicalNote::NOTE_C + i),
        ChordType::CHORD_TYPE_MAJ, ChordAccent::CHORD_ACC_NONE, 0);
      self->num_chords++;
    }
...
chord_editor_clone (ChordEditor * src)
{
  ...
  for (int i = 0; i < src->num_chords; i++)
    {
      self->chords[i] = chord_descriptor_clone (src->chords[i]);
    }
  ...
  bool                    undoable)
{
  if (undoable)
    {
      GError * err = NULL;
      bool     ret = chord_action_perform (NULL, NULL, chord, idx, &err);
      g_return_if_fail (ret);
    }
  else
    {
      chord_descriptor_copy (self->chords[idx], chord);
      chord_descriptor_update_notes (self->chords[idx]);
    }
}
  ...
  bool                     undoable)
{
  if (undoable)
    {
      ...
      bool     ret = chord_action_perform (
        (const ChordDescriptor **) self->chords, chords, NULL, -1, &err);
      g_return_if_fail (ret);
    }
  else
    {
  ...
  EVENTS_PUSH (EventType::ET_CHORDS_UPDATED, NULL);
}
...
{
  chord_editor_apply_chords (
    self, (const ChordDescriptor **) pset->descr, undoable);
}
  ...
  bool             undoable)
{
  ...
  const ChordType * triads = musical_scale_get_triad_types (scale, true);
  const bool *      notes = musical_scale_get_notes (scale, true);
  int               cur_chord = 0;
  ...
  while (cur_chord < CHORD_EDITOR_NUM_CHORDS)
    {
      new_chords[cur_chord++] = chord_descriptor_new (
        MusicalNote::NOTE_C, false, MusicalNote::NOTE_C,
        ChordType::CHORD_TYPE_NONE, ChordAccent::CHORD_ACC_NONE, 0);
    }

  chord_editor_apply_chords (
    self, (const ChordDescriptor **) new_chords, undoable);
...
chord_editor_transpose_chords (ChordEditor * self, bool up, bool undoable)
{
  ...
  for (int i = 0; i < 12; i++)
    {
      new_chords[i] = chord_descriptor_clone (self->chords[i]);
      ChordDescriptor * descr = new_chords[i];

      int add = (up ? 1 : 11);
      descr->root_note = (MusicalNote) ((int) descr->root_note + add);
      descr->bass_note = (MusicalNote) ((int) descr->bass_note + add);

      descr->root_note = ENUM_INT_TO_VALUE (
        MusicalNote, ENUM_VALUE_TO_INT (descr->root_note) % 12);
      descr->bass_note = ENUM_INT_TO_VALUE (
        MusicalNote, ENUM_VALUE_TO_INT (descr->bass_note) % 12);
    }
...
chord_editor_free (ChordEditor * self)
{
  for (int i = 0; i < self->num_chords; i++)
    {
      object_free_w_func_and_null (chord_descriptor_free, self->chords[i]);
    }
