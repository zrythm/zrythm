  ...
  for (int i = 127; i >= 0; i--)
    {
      ...
      descr->note_name = g_strdup_printf (
        "%s%d",
        chord_descriptor_note_to_string (ENUM_INT_TO_VALUE (MusicalNote, i % 12)),
        i / 12 - 1);
...
{
  if (!array_contains_int (self->current_notes, self->num_current_notes, note))
    {
      array_append (self->current_notes, self->num_current_notes, note);
    }
}
...
{
  return array_contains_int (self->current_notes, self->num_current_notes, note);
}
...
piano_roll_init_loaded (PianoRoll * self)
{
  if (!ZRYTHM_TESTING)
    {
      self->highlighting = ENUM_INT_TO_VALUE (
...
void
piano_roll_set_highlighting (PianoRoll * self, PianoRollHighlighting highlighting)
{
  ...
  EVENTS_PUSH (EventType::ET_PIANO_ROLL_HIGHLIGHTING_CHANGED, NULL);
}
...
piano_roll_set_notes_zoom (PianoRoll * self, float notes_zoom, int fire_events)
{
  ...
  if (fire_events)
    {
      EVENTS_PUSH (EventType::ET_PIANO_ROLL_KEY_ZOOM_CHANGED, NULL);
    }
}
...
piano_roll_set_midi_modifier (PianoRoll * self, MidiModifier modifier)
{
  ...
  EVENTS_PUSH (EventType::ET_PIANO_ROLL_MIDI_MODIFIER_CHANGED, NULL);
}
...
piano_roll_init (PianoRoll * self)
{
  ...
  self->midi_modifier = MidiModifier::MIDI_MODIFIER_VELOCITY;
  ...
  if (!ZRYTHM_TESTING)
    {
      self->highlighting = ENUM_INT_TO_VALUE (
        PianoRollHighlighting,
        g_settings_get_enum (S_UI, "piano-roll-highlight"));
      self->midi_modifier = ENUM_INT_TO_VALUE (
