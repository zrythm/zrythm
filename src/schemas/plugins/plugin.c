...
Plugin *
plugin_upgrade_from_v1 (Plugin_v2 * old)
{
  if (!old)
    return NULL;
  ...
  for (int i = 0; i < old->num_in_ports; i++)
    {
      self->in_ports[i] = old->in_ports[i];
    }
  self->num_out_ports = old->num_out_ports;
  self->out_ports = g_malloc_n ((size_t) self->num_out_ports, sizeof (Port *));
  for (int i = 0; i < old->num_out_ports; i++)
    {
      self->out_ports[i] = old->out_ports[i];
    }
  self->num_banks = old->num_banks;
  self->banks =
    g_malloc_n ((size_t) self->num_banks, sizeof (PluginPresetIdentifier *));
  for (int i = 0; i < old->num_banks; i++)
    {
      self->banks[i] = old->banks[i];
    }
  self->selected_bank = old->selected_bank;
  self->selected_preset = old->selected_preset;
  self->visible = old->visible;
  self->state_dir = old->state_dir;
