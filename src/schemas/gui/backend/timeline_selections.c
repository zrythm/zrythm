...
#define UPDATE(name) self->name = old->name
  ...
  ArrangerSelections * base = arranger_selections_upgrade_from_v1 (&old->base);
  self->base = *base;
  ...
  self->scale_objects = g_malloc_n ((size_t) self->num_scale_objects, sizeof (ZRegion *));
  for (int i = 0; i < self->num_scale_objects; i++)
    {
      self->scale_objects[i] = scale_object_upgrade_from_v1 (old->scale_objects[i]);
    }
  self->num_markers = old->num_markers;
  self->markers = g_malloc_n ((size_t) self->num_markers, sizeof (ZRegion *));
