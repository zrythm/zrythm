...
#define UPDATE_PORT(name) self->name = port_upgrade_from_v1 (old->name)
  ...
  self->ext_audio_ports =
    g_malloc_n ((size_t) self->num_ext_audio_ports, sizeof (ExtPort *));
  ...
  self->ext_midi_ports =
    g_malloc_n ((size_t) self->num_ext_midi_ports, sizeof (ExtPort *));
  ...
  self->audio_ports =
    g_malloc_n ((size_t) self->num_audio_ports, sizeof (Port *));
