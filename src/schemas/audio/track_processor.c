...
TrackProcessor *
track_processor_upgrade_from_v1 (
  TrackProcessor_v1 * old)
{
  if (!old)
    return NULL;
  ...
  for (int i = 0; i < 16; i++)
    {
      ...
      self->poly_key_pressure[i] = port_upgrade_from_v1 (old->poly_key_pressure[i]);
      self->channel_pressure[i] = port_upgrade_from_v1 (old->channel_pressure[i]);
    }
