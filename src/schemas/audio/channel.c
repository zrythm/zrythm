  ...
  Channel_v1 * old)
{
  ...
  for (int i = 0; i < EXT_PORTS_MAX; i++)
    {
      self->ext_midi_ins[i] = ext_port_upgrade_from_v1 (old->ext_midi_ins[i]);
      self->ext_stereo_l_ins[i] = ext_port_upgrade_from_v1 (old->ext_stereo_l_ins[i]);
      self->ext_stereo_r_ins[i] = ext_port_upgrade_from_v1 (old->ext_stereo_r_ins[i]);
    }
