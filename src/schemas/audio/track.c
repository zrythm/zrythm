...
track_upgrade_from_v1 (Track_v1 * old)
{
  ...
  self->lanes = g_malloc_n (
    (size_t) self->num_lanes, sizeof (TrackLane *));
  for (int i = 0; i < self->num_lanes; i++)
    {
      self->lanes[i] =
        track_lane_upgrade_from_v1 (old->lanes[i]);
    }
  self->num_chord_regions = old->num_chord_regions;
  self->chord_regions = g_malloc_n (
    (size_t) self->num_chord_regions, sizeof (ZRegion *));
  for (int i = 0; i < self->num_chord_regions; i++)
    {
      self->chord_regions[i] =
        region_upgrade_from_v1 (old->chord_regions[i]);
    }
  self->num_scales = old->num_scales;
  self->scales = g_malloc_n (
    (size_t) self->num_scales, sizeof (ScaleObject *));
  for (int i = 0; i < self->num_scales; i++)
    {
      self->scales[i] =
        scale_object_upgrade_from_v1 (old->scales[i]);
    }
  self->num_markers = old->num_markers;
  self->markers = g_malloc_n (
    (size_t) self->num_markers, sizeof (Marker *));
  ...
  self->beats_per_bar_port =
    port_upgrade_from_v1 (old->beats_per_bar_port);
  self->beat_unit_port =
  ...
  for (int i = 0; i < self->num_modulators; i++)
    {
      self->modulators[i] =
        plugin_upgrade_from_v1 (old->modulators[i]);
    }
  self->num_modulator_macros = old->num_modulator_macros;
  for (int i = 0; i < self->num_modulator_macros; i++)
    {
      self->modulator_macros[i] =
        modulator_macro_processor_upgrade_from_v1 (
          old->modulator_macros[i]);
    }
  self->num_visible_modulator_macros =
    old->num_visible_modulator_macros;
  self->processor =
    track_processor_upgrade_from_v1 (old->processor);
  ...
  self->record_set_automatically =
    old->record_set_automatically;
