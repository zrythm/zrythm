  ...
  Track_v1 * old)
{
  ...
  self->chord_regions = g_malloc_n ((size_t) self->num_chord_regions, sizeof (ZRegion *));
  ...
  for (int i = 0; i < self->num_modulator_macros; i++)
    {
      self->modulator_macros[i] = modulator_macro_processor_upgrade_from_v1 (old->modulator_macros[i]);
    }
  ...
  AutomationTracklist * atl = automation_tracklist_upgrade_from_v1 (&old->automation_tracklist);
