          ...
          if (type == ResizeType::Loop)
            {
              ...
              if (clip_start_pos >= loop_start_pos)
                {
                  clip_start_pos.add_ticks (ticks);

                  while (clip_start_pos < loop_start_pos)
                    {
                      clip_start_pos.add_ticks (loop_len);
                    }
  ...
  if (can_loop ())
    {
      ...
      loopable_object->loop_end_pos_.from_ticks (ticks);
    }
  if (can_fade ())
    {
      auto ticks = get_length_in_ticks ();
      auto fadeable_object = dynamic_cast<FadeableObject *> (this);
      fadeable_object->fade_out_pos_.from_ticks (ticks);
    }
}
        ...
        if constexpr (RegionSubclass<SelfT>)
          {
            ...
            local.from_frames (local_frames);
          }
