...
region_musical_mode_to_str (RegionMusicalMode mode)
{
  ...
  return region_musical_mode_strings[ENUM_VALUE_TO_INT (mode)];
}
...
{
  g_return_if_fail (
    ENUM_VALUE_TO_INT (type) >= 0 && type <= RegionType::REGION_TYPE_CHORD);
  switch (type)
    {
    case RegionType::REGION_TYPE_MIDI:
      strcpy (buf, _ ("MIDI"));
...
region_find (const RegionIdentifier * const id)
{
  ...
  if (
    id->type == RegionType::REGION_TYPE_MIDI
    || id->type == RegionType::REGION_TYPE_AUDIO)
    {
  ...
  else if (id->type == RegionType::REGION_TYPE_AUTOMATION)
    {
      track = tracklist_find_track_by_name_hash (TRACKLIST, id->track_name_hash);
      g_return_val_if_fail (track, NULL);
  ...
  else if (id->type == RegionType::REGION_TYPE_CHORD)
    {
      track = P_CHORD_TRACK;
      g_return_val_if_fail (track, NULL);
...
region_get_musical_mode (ZRegion * self)
{
  ...
  switch (self->musical_mode)
    {
    case RegionMusicalMode::REGION_MUSICAL_MODE_INHERIT:
      return g_settings_get_boolean (S_UI, "musical-mode");
    case RegionMusicalMode::REGION_MUSICAL_MODE_OFF:
      return false;
    case RegionMusicalMode::REGION_MUSICAL_MODE_ON:
      return true;
    }
