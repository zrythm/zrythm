...
automation_record_mode_get_localized (AutomationRecordMode mode, char * buf)
{
  switch (mode)
    {
    case AutomationRecordMode::AUTOMATION_RECORD_MODE_TOUCH:
      strcpy (buf, _ ("Touch"));
      break;
    case AutomationRecordMode::AUTOMATION_RECORD_MODE_LATCH:
      strcpy (buf, _ ("Latch"));
      break;
    default:
      g_return_if_reached ();
              ...
              if (dest->owner_type == ZPortOwnerType::Z_PORT_OWNER_TYPE_PLUGIN)
                {
                  ...
                  if (
                    ...
                    == ZPluginProtocol::Z_PLUGIN_PROTOCOL_LV2)
                    {
  ...
  bool              fire_events)
{
  ...
  if (mode == AutomationMode::AUTOMATION_MODE_RECORD)
    {
  ...
  if (fire_events)
    {
      EVENTS_PUSH (EventType::ET_AUTOMATION_TRACK_CHANGED, self);
    }
}
...
{
  if (G_LIKELY (at->automation_mode != AutomationMode::AUTOMATION_MODE_RECORD))
    return false;

  if (at->record_mode == AutomationRecordMode::AUTOMATION_RECORD_MODE_LATCH)
    {
  ...
  else if (at->record_mode == AutomationRecordMode::AUTOMATION_RECORD_MODE_TOUCH)
    {
      Port * port = at->port;
