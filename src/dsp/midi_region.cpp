...
midi_region_insert_midi_note (
  ...
  int        pub_events)
{
  g_return_if_fail (self->id.type == RegionType::REGION_TYPE_MIDI);
  ...
  if (pub_event)
    {
      EVENTS_PUSH (
        ...
        ArrangerObjectType::ARRANGER_OBJECT_TYPE_MIDI_NOTE);
    }
}
  ...
  bool                                is_note_off_for_loop_or_region_end)
{
  ...
  if (self->id.type == RegionType::REGION_TYPE_MIDI)
    {
      channel = midi_region_get_midi_ch (self);
    }
  else if (self->id.type == RegionType::REGION_TYPE_CHORD)
    {
      /* FIXME set channel */
    }
