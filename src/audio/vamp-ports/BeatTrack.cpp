...
BeatTracker::getParameterDescriptors () const
{
  ...
  desc.description =
    "User-defined tempo on which to centre the tempo preference function";
  desc.minValue = 50;
  desc.maxValue = 250;
  ...
  desc.description =
    "Constrain more tightly around the tempo hint, using a Gaussian weighting instead of Rayleigh";
  desc.minValue = 0;
  desc.maxValue = 1;
  desc.defaultValue = 0;
  desc.isQuantized = true;
  ...
  size_t blockSize)
{
  ...
  if (
    ...
    {
      std::cerr
        << "BeatTracker::initialise: Unsupported channel count: "
        << channels << std::endl;
      return false;
    }
  ...
  Vamp::RealTime        timestamp)
{
  ...
  double output =
    m_d->df->processFrequencyDomain (reals, imags);
  ...
  returnFeatures[1].push_back (
    feature); // detection function is output 1
...
BeatTracker::beatTrackOld ()
{
  ...
  vector<double> tempi;
  vector<int>    beats =
    tempoTracker.process (m_d->dfOutput, &tempi);
  ...
  for (size_t i = 0; i < beats.size (); ++i)
    {
      ...
      returnFeatures[0].push_back (
        feature); // beats are output 0
    }
  ...
  for (size_t i = 0; i < tempi.size (); ++i)
    {
      ...
      if (
             ...
             != int (prevTempo * 100))
        {
          ...
          feature.label = label;
          returnFeatures[2].push_back (
            feature); // tempo is output 2
          prevTempo = tempi[i];
        }
    }
...
BeatTracker::beatTrackNew ()
{
  ...
  tt.calculateBeats (
    df, beatPeriod, beats, m_alpha, m_tightness);
