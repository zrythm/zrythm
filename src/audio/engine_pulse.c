  ...
  void      * userdata)
{
  ...
  bytes = MIN (
    bytes,
    FRAMES_TO_BYTES (self->block_length));
  ...
  if (pa_stream_write (
        stream, buf, bytes,
        should_free_buffer ? g_free : NULL,
        0, PA_SEEK_RELATIVE) != 0)
    {
      g_warning (
        "Failed to write pulse buffer");
  ...
  gboolean      activate)
{
  ...
  while (pa_operation_get_state (op)
