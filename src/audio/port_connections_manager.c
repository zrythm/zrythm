  ...
  PortConnection *       conn)
{
  ...
  if (connections)
    {
      ...
      g_ptr_array_extend (
        connections, prev_connections, NULL,
        NULL);
  ...
  if (replacing)
    {
      PortIdentifier * pi_clone =
        port_identifier_clone (pi);
      g_hash_table_replace (
        ht, pi_clone, connections);
    }
  else
    {
      PortIdentifier * pi_clone =
  ...
  const void * user_data)
{
  const PortConnection * conn =
    (const PortConnection *) obj;
  const ChannelSend * send =
    (const ChannelSend *) user_data;
  ...
  const int                idx)
{
  ...
  for (int i = idx; i < self->num_connections - 1;
       i++)
    {
  ...
  for (int i = 0; i < self->num_connections; i++)
    {
      PortConnection * conn =
        self->connections[i];
      if (port_identifier_is_equal (conn->src_id, pi)
          ||
...
void
port_connections_manager_print_ht (
  GHashTable * ht)
{
  GHashTableIter iter;
  gpointer key, value;
